...
| `multi_value_lookup_condition_unverified` | error | A condition tests a multi-value lookup column. The multi-value CAML grammar this tool emits was measured against a Choice (multi-valued) column on 2026-08-10 and against nothing else, so what `<Eq>` means against a Lookup (multi-valued) column is unknown: a filter built on the assumption would save, read back intact and could return the wrong rows with no error. Filter on a single-value lookup, or on a projected or scalar column, until a probe settles it. |

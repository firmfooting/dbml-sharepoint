...
| `title` | Optional explicit display title, independent of the prefix and DBML entity name |
| `internal_name` | Optional, library only; the URL name used at creation. Use 1-128 letters, digits or underscores, starting with a letter. Deploy verifies the root URL and refuses mismatches; it does not move an existing library |
...
### The site's time zone is not declared here

The zone a pack converts timestamps by is a fact about the **site** it is
built for, not about the solution, so it is a build input:
[`--time-zone`](cli.md#the-sites-time-zone) on `build` and `report`, or
`DBMLSP_TIME_ZONE` in `dbml-sharepoint.env`. A mapping says *use the
site's date* (a [`derived_columns`](#derived_columns) `expr` calling
`AsSiteDate([Created])`), and the build says which site. The removed
`reporting.time_zone` key is a hard load error naming the flag; there is
no compatibility mode. What the zone gives every list query (the
daylight-saving transitions, the `AsSiteDate` and `AsSiteDateTime`
helpers, and `DateZoneResolved`'s check that the site agrees) is
described on the [reporting pack](../artifacts/reporting.md#timestamps-convert-by-the-declared-zone)
page.
...
- A `when` on a **boolean** column. SharePoint refuses a validation formula
  on a Yes/No field, and the formula is how conditional visibility is
  carried, so the deploy stops at that field with HTTP 500 *"This field type
  does not support validation formulas"* (measured on a live tenant,
  2026-09-04). Declare the visibility without `when`, or move the column off
  `boolean`. (`form_visibility_condition_on_a_boolean_column`.) Only the
  condition is refused: `new:` and `existing:` on a boolean column are not
  known to fail and are left alone.

...
# `dbml_sharepoint.ordering`

*Analysis — dependency ordering and site filtering*
...
```python
def compute_phases(schema: dbml_sharepoint.parser.Schema) -> dbml_sharepoint.ordering.DeployPlan
```

### `site_tables_in_order`

```python
def site_tables_in_order(schema: dbml_sharepoint.parser.Schema, entities: 'dict[str, EntityMapping]', site_role: str) -> list[str]
```

...
Probes the site's capabilities against this pack's requirements and prints a COMPATIBLE / DEGRADED / BLOCKED verdict. Makes NO changes: every call is a GET except the contextinfo digest fetch, one read-only CSOM ProcessQuery, and the OData $batch POSTs that carry those GETs, whose parts are all GETs and which the transport below cannot spell any other way.
...
Included by: `deploy/_logging.js.j2`

The cross-web form of apiUrl(): same suffix discipline, pointed at ANOTHER site under this tenant root. Included ONLY by deploy.js's logging phase, the one place a script reaches the central deployment log, so no other emitted script carries the helper unused. Same origin, so the session cookies ride along. TENANT_ROOT comes from SITE_URL's origin, deliberately NOT from `_spPageContextInfo`: the modern pages this is pasted on (CollabHome) do not define that global (measured 2026-08-26). TWO builders, because a form digest is NOT web-agnostic. `crossWebApi` is the `web/`-scoped one every read and write uses; `crossWebDigestUrl` is the contextinfo endpoint, which sits at the API root rather than under the web scope, and must be the TARGET web's own (live finding 2026-09-05: a digest issued by the pasted page's web answered 403 "security validation ... invalid" against another web; that web's own digest answered 201).
...
The WRITE half of the shared OData $batch transport: BatchWriter, which collapses many single writes into one ChangeSet. It includes _http_batch_read.js.j2 for the boundary and byte helpers, the body budget and BatchReader, so a script including this partial gets the same transport it got before the read half was split out. Included only by scripts that make writes (deploy, rollback, demo); the read-only assess script includes the read half alone. Expects `spHeaders` from _http_write.js.j2, so a ChangeSet part carries exactly the headers a single write does, plus `isThrottled`, `spError` and `dbg` from _http.js.j2. A part's verb goes in its request line rather than in an X-HTTP-Method header, so a caller passes the method and headers of the single write it replaces and the part still comes out in the documented shape. getDigest, fetchWithRetry, apiUrl and log arrive as CONSTRUCTOR ARGUMENTS rather than off the enclosing scope, so this stays a transport primitive with nothing to reimplement. The body-size ceiling this budgets against was measured by the same #404 probe recorded in _http_batch_read.js.j2. That probe's control read the item count back and confirmed the ChangeSet parts were accepted, so the encoding below is the proven spelling rather than a plausible one, digest on both the outer request and every part included. The PART ENCODING was measured separately, recorded 2026-09-04, after a live governance deploy landed every POST part and refused every MERGE part with HTTP 400. Nine candidate spellings were sent one per $batch against a scratch list and each verdict read back off the object rather than off the part status. Two things decide acceptance, and the earlier reading of that 400 had them the wrong way round: 1. The VERB MUST BE IN THE REQUEST LINE. X-HTTP-Method does not tunnel inside a part. SharePoint reads the part as the verb it was given, so a tunnelled MERGE arrives as a POST to the entity and its body keys are read as method arguments: "The parameter Description does not exist in method GetById". `POST` + X-HTTP-Method: MERGE was refused under both nometadata and verbose parts. 2. THE BODY ANNOTATION MUST MATCH THE PART'S CONTENT TYPE. `__metadata` is the verbose transport annotation, so a part declaring odata=nometadata is refused with "The property '__metadata' does not exist on type 'SP.FieldText'". `MERGE <url>` with verbose part headers and a __metadata body was accepted (HTTP 204, read back applied), as was a bodyless function POST under verbose part headers. odata=verbose is therefore NOT what a part is refused for; the earlier finding to that effect was drawn from a shape that also carried the tunnelled header, which is what SharePoint was actually refusing. Parts carry spHeaders for that reason.

### `_http_batch_read.js.j2`

Included by: `_http_batch.js.j2`, `assess.js.j2`

The READ half of the shared OData $batch transport: the boundary and byte helpers, the body budget both halves measure against, and BatchReader for top-level query parts, which a phase uses to read N objects in one request instead of N. Its own partial so the read-only assess script can include it on its own. Nothing in here can write, and that is what keeps assess's no-write property auditable from the text of assess.js.j2 rather than from a claim about what a shared partial happens to contain. _http_batch.js.j2 includes this file, so a write-capable script still gets both halves from the one include it always had. Expects `isThrottled`, `spError` and `dbg` from _http.js.j2. NOT `spHeaders` from _http_write.js.j2, which is the write half's dependency: a query part carries no body and no header beyond Accept. getDigest, fetchWithRetry, apiUrl and log arrive as CONSTRUCTOR ARGUMENTS rather than off the enclosing scope, so this stays a transport primitive with nothing to reimplement. MEASURED on a live tenant by test/manual/throttle-batch-probe.js (#404), recorded 2026-09-04: 20, 100, 200, 500 and 750-operation batches all landed clean, and a 1000-operation batch came back HTTP 200 with 637 parts at 201 and 363 at 500. A clean 750 beside a partial 1000 says the ceiling is body SIZE and not operation count, which is why this budgets bytes.
...
Phase body: decide where this run's record goes, ensure whatever that answer needs, and declare the change-log writer every later phase calls. ONE SINK PER RUN, decided ONCE, here. The central Deployments list is probed FIRST and the answer picks a mode for the whole run: - CENTRAL. The central site holds the Deployments list, this account can add items to it, and it carries the columns a full stamp needs. Stamps go there; type-2 change rows go to the central Changes list beside it, probed the same way but INDEPENDENTLY, so a change row can be absent without ever touching a stamp. The per-site sidecars are neither created nor written. A site that has a central log ends the run with no new lists on it at all. - LOCAL. The central site or the Deployments list is unreachable. The two sidecars below are created and written exactly as they were before the central log existed. Only the DEPLOYMENTS probe decides LOG_MODE. The Changes list can never promote a run to CENTRAL or demote it to LOCAL: a Changes list that is absent, unreachable, or short a declared column costs the change feed alone, and the run's stamps are unaffected either way. The mode never changes mid-run. A CENTRAL write that fails is recorded on summary.loggingFailures and the run carries on; it does NOT fall back to the sidecars, because a record half in one place and half in another is worse than either place alone, and it is the failure this design exists to prevent. THE FOUR LISTS, four different contracts: - `dbml_Deployments` (LOCAL only) is hidden and left with no role assignments, so an ordinary member sees nothing. It records this run: start stamp, stop stamp, and what the run did. Title carries the readable line and the stamp columns carry the same facts queryably. A log the marker matches is REUSED, and the ones already on live sites were created before those columns existed, so the phase creates the ones it finds missing and falls back to a Title-only stamp when it cannot. - The change log (default `dbml_Changes`, LOCAL only) is hidden and holds one Read assignment for the site group flagged `enroll_enterprise_reader`. It receives type-2 rows: every change carries its key in ChangeKey, plus EffectiveFrom/EffectiveTo and IsCurrent, and a change CLOSES the previous current row for its key in the same logical step, so a reader always sees exactly one current row per key. ChangeKey and IsCurrent are indexed because the close reads them both in one $filter, and both sides of an AND have to be indexed to survive the 5,000-item threshold on a log that grows forever. - The CENTRAL Deployments list (default `firmfooting_Deployments`) is NEVER created by a deploy. It carries only stamp rows now: `deployment start`, `deployment stop`, `abort` and `provenance`. Absent, this phase says so once and drops the whole run to LOCAL. - The CENTRAL Changes list (default `firmfooting_Changes`), beside it on the same site, is also NEVER created by a deploy. It carries the type-2 change feed that used to share the Deployments list under `StampKind: 'change'`; that member is retired now that a change row has its own list and no StampKind column to carry it. Absent, unreachable, or short a declared column, this phase says so once and the run's changes are counted and dropped rather than written to the sidecars, because a change feed split across two lists is unreadable in both. The two LOCAL lists carry the tool's provenance marker in their Description. A list of the same title whose Description is not EXACTLY the marker is somebody else's and this phase ABORTS the run, the same fail-closed contract as verify.js's scratch list. Creation is idempotent; a rollback never deletes these lists because rollback only deletes declared TARGET_LISTS. `--no-sidecars` suppresses the LOCAL half and nothing else. With a central log reachable it changes nothing at all, because that run was never going to create a sidecar. In LOCAL mode it means this run keeps no record: stamps are skipped and change rows are counted and dropped. Change events raised BEFORE this phase (renames, phase 1.3) were buffered by deploy.js.j2's shim and are drained once the writer is live. If the run aborts before reaching this phase they are lost, and so is everything else about that run.

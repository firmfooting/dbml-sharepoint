  ...
  for (const list of SCHEMA.lists) {
    try {
      ...
      if (listShape) {
        ...
        // Learn: determine-sharepoint-rest-service-endpoint-uris, ListCollection.Add.
        const created = list.internal_name
          ? await postJson(apiUrl('web/lists/add'), { parameters: {
              __metadata: { type: 'SP.ListCreationInformation' },
              Title: list.title, Url: list.internal_name,
              TemplateType: list.base_template, Description: list.description || '',
            } }, digest)
          : await postJson(apiUrl('web/lists'), body, digest);
      ...
      // MEASURED 2026-09-22, operator-safety-grant-probe.js, on two sites:
      // the break left exactly one role assignment, this account's own direct
      // USER binding (PrincipalType 1) at Full Control, and that binding is
      // removable and stays removed. clearSubscopes stays unmeasured.

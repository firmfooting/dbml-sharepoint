  ...
  async function readFolder(serverRelativeUrl) {
    ...
    if (!folder || typeof folder !== 'object' || Array.isArray(folder)
      || typeof folder.Exists !== 'boolean') throw new Error('folder read failed: malformed folder response');
    if (!folder.Exists) return null;
    if (folder.ServerRelativeUrl !== serverRelativeUrl) throw new Error('folder read failed: mismatched folder path');
    // https://learn.microsoft.com/en-us/sharepoint/dev/sp-add-ins/working-with-folders-and-files-with-rest
    const itemRead = await fetchWithRetry(apiUrl(`web/GetFolderByServerRelativeUrl('${pathLiteral(serverRelativeUrl)}')/ListItemAllFields?$select=FileSystemObjectType,FileRef`), {
      headers: { 'Accept': 'application/json;odata=verbose' },
    });
    if (!itemRead.ok) throw new Error(`folder item probe failed: HTTP ${itemRead.status}`);
    const itemJson = await itemRead.json();
    const item = itemJson && itemJson.d;
    if (!item || typeof item !== 'object' || Array.isArray(item)
      || item.FileSystemObjectType !== FOLDER_OBJECT_TYPE || item.FileRef !== serverRelativeUrl) {
      throw new Error('folder item probe failed: missing or mismatched item, or not a folder');
    }
    return folder;
  }
  async function folderItemShape(listTitle, rootUrl, name) {
    const filter = encodeURIComponent(`FileLeafRef eq '${String(name).replace(/'/g, "''")}'`);
    let url = apiUrl(`web/lists/getbytitle('${odataName(listTitle)}')/items?$select=Id,FileSystemObjectType,FileLeafRef,FileRef&$filter=${filter}`);
    ...
    while (url) {
      ...
      if (!page || !Array.isArray(page.results) || page.results.some((row) =>
        ...
        || ![0, FOLDER_OBJECT_TYPE].includes(row.FileSystemObjectType)
        || typeof row.FileRef !== 'string' || !row.FileRef.startsWith('/')
        || row.FileRef.endsWith('/'))) {
        throw new Error('folder item probe failed: missing or malformed folder collection');
      }
      url = validatedNextPage(page, 'folder item probe');
      ...
      for (const row of page.results) {
        ...
        if (match) throw new Error('folder item probe failed: ambiguous root folder collection');
        match = row;
      }
    }
    return match;
  ...
  for (const list of SCHEMA.lists.filter((l) => l.is_library && l.folders.length)) {
    ...
    try {
      ...
      if (typeof rootUrl !== 'string' || !rootUrl.startsWith('/') || rootUrl.endsWith('/')) {
        throw new Error('RootFolder read back no valid ServerRelativeUrl');
      }
    } catch (err) {
      ...
      for (const name of missing) {
        ...
        try {
          ...
          try {
            ...
            let blocking;
            try {
              blocking = await folderItemShape(list.title, rootUrl, name);
            } catch (probeErr) {
              throw new Error(`${err.message}; collision diagnosis unavailable: ${probeErr.message}`);
            }

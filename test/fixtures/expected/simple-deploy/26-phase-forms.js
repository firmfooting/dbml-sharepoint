    ...
    if (SCHEMA.form_formatting.length > 0) {
      ...
      try {
        ...
        for (const form of SCHEMA.form_formatting) {
          // `$top` for the same reason every other enumeration carries
          // it: no explicit page size means the server's, and a truncated
          // read here reads as "this list has no such content type".
          await ctReader.add(`${formListPath(form.list)}/contenttypes?$select=Name,StringId,ClientFormCustomFormatter&$top=${CONTENT_TYPE_PAGE_SIZE}`);
        }
      ...
      for (let position = 0; position < SCHEMA.form_formatting.length; position += 1) {
        ...
        try {
          ...
          const next = validatedNextPage(contentTypes[position], `Content-type read for '${form.list}'`);
          ...
          // A missing target authorises no write, so only the message changes on a truncated page (#577).
          if (!target && pageTruncated(listed, next, CONTENT_TYPE_PAGE_SIZE)) {
            throw new Error(`no default item content type on the first page of ${CONTENT_TYPE_PAGE_SIZE}`
              + ' content types, and the page may be truncated, so it may be past the page rather than missing');
          }
    ...
    if (formTargets.length > 0) {
      ...
      for (let position = 0; position < formTargets.length; position += 1) {
        ...
        try {
          ...
          // A miss here cannot mean the list vanished: an absent list fails
          // the survey, which returns null and takes the branch above.
          const listId = surveyedListId(verifyOwned, form.list, 'Form read-back');

  ...
  // Both collections: the operator needs rights on a library this bundle
  // secures at folder scope exactly as on one it secures at list scope.
  for (const listTitle of [...new Set(
    [...SCHEMA.list_assignments, ...SCHEMA.folder_assignments].map((la) => la.list),
  )]) {
    try {

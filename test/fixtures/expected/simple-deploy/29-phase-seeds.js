...

  {
    ...
    for (const seed of SCHEMA.seed_items) {
      ...
      try {
        const seedListId = surveyedListId(seedOwned, seed.title, 'Seed');
  ...
  // Every list any scope names: the operator needs rights on a library this
  // bundle secures at folder scope exactly as on one it secures at list scope.
  for (const listTitle of [...new Set(SCHEMA.acl_scopes.map((s) => s.list))]) {
    try {

  ...
  // them.
  //
  // `expectedId` takes no default and may not be nullish. A default fires on
  // `undefined`, so `ownedListIdentity(title, owned.get(title))` for a title
  // the survey never covered bound the default and skipped the comparison
  // entirely, leaving the marker-only check the comment above says is not
  // enough. The two cases are indistinguishable from inside the function, so
  // absence is refused here rather than told apart.
  async function ownedListIdentity(listName, expectedId, when = 'before a write') {
    if (expectedId == null) {
      throw new Error(`No proven identity was captured for list '${listName}' ${when}`);
    }
    const actual = await assertDeclaredListOwnedNow(listName);
    const listId = sharePointGuid(actual.Id, 'list');
    if (listId !== sharePointGuid(expectedId, 'list')) {
  ...
  // legitimately run before a list exists; absence is not a failure there,
  // and those callers get a Map that may be short.
  async function surveyOwnedListsForWrites(listTitles, phaseNumber, label, allowAbsent = false) {
    ...
    // Settled here, not at `surveyedListId`: its throw lands in a per-target
    // catch that drops one target and writes the rest. `reported` holds the
    // titles the lane already explained, so one incident is one error.
    if (!allowAbsent) {
      for (const listTitle of new Set(listTitles)) {
        if (identities.get(listTitle) != null || reported.has(listTitle)) continue;
  ...
  // One sentence for both refusals, so they cannot name one fault two ways.

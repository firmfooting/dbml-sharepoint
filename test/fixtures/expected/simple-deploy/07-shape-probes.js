  ...
  // A malformed continuation cannot establish that a collection ended.
  function validatedNextPage(page, what) {
    const next = page && page.__next;
    if (next != null && typeof next !== 'string') {
      throw new Error(`${what}: invalid OData __next continuation`);
    }
    return next || null;
  }

  // An incomplete name index must fall back to direct existence probes.
  function completeNamePage(json) {
    const page = json && json.d;
    if (!page || !Array.isArray(page.results)
        || page.results.length >= 5000
        || page.results.some(row => !row || typeof row.Title !== 'string')
        || (page.__next != null && page.__next !== '')) return null;
    return page.results;
  }
  ...
  async function ensureKnownListTitles(force = false) {
    ...
    if (rows === null) { knownListTitles = null; return null; }
    knownListTitles = nameSet(rows.map(row => row.Title));
  ...
  async function probeListShapeByTitle(name) {
    ...
    const declared = SCHEMA.lists.find(list => nameKey(list.title) === nameKey(name)
      || (list.renamed_from || []).some(old => nameKey(old.title) === nameKey(name)));
    const rootSelect = declared?.internal_name ? ',RootFolder/ServerRelativeUrl&$expand=RootFolder' : '';
    const select = [
    ...
    const r = await fetchWithRetry(apiUrl(`web/lists/getbytitle('${odataName(name)}')?$select=${select}${rootSelect}`), {
      headers: { 'Accept': 'application/json;odata=verbose' },
    });
  ...
  async function listFieldShapes(listName) {
    ...
    const j = await r.json();
    const next = validatedNextPage(j && j.d, `Field enumeration for '${listName}'`);
    ...
    const shapes = {
      ...
      // either way.
      truncated: fieldPageTruncated(rows, next),
    };

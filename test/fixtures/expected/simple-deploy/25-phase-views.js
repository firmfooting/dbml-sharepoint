  ...
  async function listViewShapes(listPath) {
    if (!(listPath in viewShapesByList)) {
      ...
      const r = await fetchWithRetry(apiUrl(`${listPath}/views?$select=Id,Title,DefaultView,Hidden,RowLimit,ViewQuery,Scope,PersonalView,CustomFormatter,Aggregations,AggregationsStatus,ServerRelativeUrl,ViewFields&$expand=ViewFields&$top=${VIEW_PAGE_SIZE}`), {
        headers: { 'Accept': 'application/json;odata=verbose' },
      });
      if (!r.ok) {
        const text = await r.text();
  ...
  const deployView = async (view) => {
    try {
      ...
      const viewListId = surveyedListId(viewsOwned, view.list, 'View');

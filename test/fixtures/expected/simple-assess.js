...
(async () => {
  ...
  async function assessSite(ctx) {
    ...
    const folderShapePath = (title, name) => {
      ...
      return `web/lists/getbytitle('${odataName(title)}')/items?$select=Id,FileSystemObjectType,FileLeafRef&$filter=${filter}&$top=${FOLDER_PAGE_SIZE}`;
    };
    ...
    for (const [title, folders] of (TARGETS.library_folders || [])) {
      ...
      for (let at = 0; at < folders.length; at += 1) {
        ...
        else if (rows.d.results.length >= FOLDER_PAGE_SIZE || rows.d.__next) {
          unreadable = 'incomplete folder collection'; break;
        }
      }
      if (unreadable !== null) {

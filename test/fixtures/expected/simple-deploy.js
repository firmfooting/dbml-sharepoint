  ...
  const deployView = async (view) => {
    try {
      ...
      if (!existing) {
        ...
        // matching CustomFormatter and widths — so deleting a totals block
        // does NOT clear a deployed total.
        //
        // normalizeViewQuery is required here, not tidiness: SP reads back
        // `<FieldRef Name="X" Type="Sum" />` for the `...Type="Sum"/>` it
        // was sent, which is the pre-`/>` space that normaliser exists for.
        // Compared raw, a correct view drifts on every redeploy, rewrites,
        // reads the same difference back, and fails the phase closed — on
        // the SECOND run, never the first.
        if (view.aggregations
            && normalizeViewQuery(existing.Aggregations) !== normalizeViewQuery(view.aggregations)) {
          patchBody.Aggregations = view.aggregations;
          patchBody.AggregationsStatus = 'On';
        }

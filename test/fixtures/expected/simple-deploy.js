...
(async () => {
  ...
  const deployView = async (view) => {
    try {
      ...
      // does not send Aggregations, so a version of this that lived in the
      // `existing` arm left every FIRST deploy of a totals view failing its
      // own verify, on exactly the views the feature was built for.
      //

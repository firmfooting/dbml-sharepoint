...
(async () => {
  ...
  // here: the document closed (its `</html>` is present), and it is the size
  // of a settings page. That page measured 501,773 characters on 2026-08-17,
  // so this floor sits an order of magnitude under it and rejects a login
  // stub or an error page without resting on a size SharePoint owns.
  ...
  async function readEditorControls(view, ownedId) {
    ...
    // `</html>` need not be the final characters: SharePoint now serves
    // trailing markup after the document close, so `endsWith` read a complete
    // page as cut short. A page cut before the close carries no `</html>` at
    // all, so containment still tells a truncated page from a complete one.
    const complete = body.includes('</html>') && body.length >= EDITOR_PAGE_MIN_CHARS;
  ...
  async function confirmEditorRefusesTheGuard() {
    ...
    // view-edit-page-probe.js C1 and F7), so it says whether the markers

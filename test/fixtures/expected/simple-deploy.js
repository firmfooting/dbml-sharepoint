...
(async () => {
  ...
  // here: the document ended, and it is the size of a settings page. That
  // page measured 501,773 characters on 2026-08-17, so this floor sits an
  // order of magnitude under it and rejects a login stub or an error page
  // without resting on a size SharePoint owns.
  ...
  async function readEditorControls(view, ownedId) {
    ...
    const complete = body.trimEnd().endsWith('</html>') && body.length >= EDITOR_PAGE_MIN_CHARS;

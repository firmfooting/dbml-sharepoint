...
AGENTS.md has banned em dashes since the writing-style rules were written, and
the ban was enforced by nothing. The rule reads as being about prose, so the
...
def _relative(path: Path) -> str:
    """Repo-relative and posix-separated, because the roots are written that way."""
...
def _shipped_roots() -> tuple[str, ...]:
    """What the wheel is built from, read from packaging rather than repeated."""
...
def _non_ascii(line: str) -> str:
    """Every offending codepoint named, so the report says what to replace."""
...
def test_the_scan_reports_a_seeded_non_ascii_character(tmp_path: Path) -> None:
    """Proof the ASCII scan reports a character, independent of a clean tree."""

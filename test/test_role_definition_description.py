...
def test_the_marker_names_the_family() -> None:
    assert marker_for_level("risk-register") == (
...
def test_the_marker_opens_with_the_shared_prefix() -> None:
    ...
    assert marker_for_level("risk-register").startswith(MARKER_PREFIX)


def test_the_composed_description_is_note_then_marker() -> None:
    composed = level_description("Rates risks on a shared list.", family="risk-register")
    assert composed == (
        "Rates risks on a shared list. Provisioned by dbml-sharepoint from risk-register."
    )
...
def test_an_empty_declaration_returns_the_marker_alone() -> None:
    ...
    composed = level_description("", family="risk-register")
    assert composed == marker_for_level("risk-register")
...
def test_the_note_is_clamped_before_the_marker_is_appended() -> None:
    ...
    composed = level_description("z" * (budget + 50), family="risk-register")
    assert composed.endswith("Provisioned by dbml-sharepoint from risk-register.")
...
def test_the_budget_reserves_room_for_the_marker_to_grow() -> None:
    budget = level_description_budget("risk-register")
    marker = marker_for_level("risk-register")
    assert budget == (
...
def test_an_absurd_family_name_yields_a_zero_budget_not_a_negative_one() -> None:
    ...
    assert level_description_budget("f" * MAX_ROLE_DEFINITION_DESCRIPTION) == 0


def test_a_zero_budget_still_returns_the_marker_alone() -> None:
    family = "f" * MAX_ROLE_DEFINITION_DESCRIPTION
    composed = level_description("a note", family=family)
    assert composed == marker_for_level(family)

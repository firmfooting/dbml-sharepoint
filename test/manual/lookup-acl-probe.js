 ...
 *        CONTROL, pass 3, as the site owner: after pass 2, does the target
 *        still exist with broken inheritance, holding the row the source
 *        fixture links to? Without it a target deleted between the passes
 *        reads as a working ACL, and K2 reports a withheld value that was
 *        never there to withhold.
 *
 ...
 *   PASS 3: as the SITE OWNER again. MODE = 'confirm'. Writes NOTHING.
 *            Answers K8.
 *
 ...
 *   5. Back as the site owner, set MODE = 'confirm', paste. CONFIRMED alone
 *      is enough; it never writes.
...
(async () => {
  ...
  if (MODE === 'read') {
    ...
    if (!me.ok) {
      ...
      record('access.lookup-acl.control-target-denied', 'CONTROL: is the second account actually denied the TARGET list?',
             ...
             + `the target still there. Running as: ${who}`);
    } else {
      record('access.lookup-acl.control-target-denied', 'CONTROL: is the second account actually denied the TARGET list?',
             'PASS',
    ...
    log('INFO', "Now run pass 3 as the site owner: MODE = 'confirm'.");
    return;
  }

  // ---- PASS 3: confirm, as the site owner, after pass 2 ---------------
  // A 404 in pass 2 is a denial only if the target was there to be denied.
  // This reads, as the owner, that it still exists, still has its own
  // permissions, and still holds the very row the source fixture links to,
  // so a target deleted or rebuilt between the passes cannot pass for one.
  if (MODE === 'confirm') {
    ...
    const rows = (source.ok && source.body && source.body.value) || [];
    const fixture = rows.find((r) => r.Title === 'dbmlsp-probe-source-row');
    const linkedId = fixture ? fixture[`${LOOKUP}Id`] : null;
    if (list.status === 404) {
      record(K8, K8Q, 'FAIL',
             `the site owner reads ${TARGET} as HTTP 404: it is gone, so a 404 in pass 2 `
             + 'was an absence, not a denial, and K2 and K3 prove nothing.');
    } else if (readFailed(list) || readFailed(source)) {
      record(K8, K8Q, 'NOT ESTABLISHED',
             `the owner's reads answered HTTP ${list.status} (target) and HTTP `
             + `${source.status} (source), so neither presence nor absence was observed. Re-run.`);
    } else if (list.body.HasUniqueRoleAssignments !== true) {
      record(K8, K8Q, 'FAIL',
             `${TARGET} exists but HasUniqueRoleAssignments reads `
             + `${JSON.stringify(list.body.HasUniqueRoleAssignments)}, so the ACL pass 2 `
             + 'was meant to meet is not the one on the list now.');
    } else if (!linkedId) {
      record(K8, K8Q, 'NOT ESTABLISHED',
             `the source fixture row is ${fixture ? 'present without a lookup id' : 'missing'}, `
             + 'so there is no linked row to look for in the target. Re-run the setup pass.');
    } else {
      const row = await spGet(`web/lists/getbytitle('${TARGET}')/items(${Number(linkedId)})?$select=Id,Title`);
      record(K8, K8Q,
             row.ok && row.body && row.body.Id === Number(linkedId) ? 'PASS'
               : row.status === 404 ? 'FAIL' : 'NOT ESTABLISHED',
             row.ok && row.body && row.body.Id === Number(linkedId)
               ? `${TARGET} exists with its own permissions and holds item ${linkedId}, `
                 + 'the row the source fixture links to'
               : row.status === 404
                 ? `${TARGET} exists but item ${linkedId}, the row the source fixture links `
                   + 'to, does not: the target was rebuilt between the passes.'
                 : `reading item ${linkedId} answered HTTP ${row.status}, which is neither `
                   + 'presence nor absence. Re-run.');
    }
    report();
    log('INFO', 'Read-only pass complete. Nothing was written.');
    return;
  ...
  console.log("  4. Back as the site owner, paste with MODE = 'confirm' and CONFIRMED = true.");

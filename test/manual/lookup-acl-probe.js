  ...
  if (MODE === 'read') {
    ...
    if (isAdmin) {
      ...
      record('K1', 'CONTROL: is the second account actually denied the TARGET list?',
             target.ok ? 'FAIL' : 'PASS',
             target.ok
               ? `the account READ ${TARGET} (HTTP 200), so inheritance was not broken `
                 + 'as intended and K2/K3 prove nothing about a denied reader'
    ...
    record('K4', 'CONTROL: can the denied account read the SOURCE list at all?',
           source.ok ? 'PASS' : 'FAIL',
           source.ok
             ? `read ${rows.length} row(s) from ${SOURCE}`
    ...
    if (!source.ok || isAdmin || target.ok) {

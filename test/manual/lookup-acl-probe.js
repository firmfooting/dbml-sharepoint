 ...
 *        was never there to withhold. K1, K2 and K3 all depend on it. Only a
 *        site collection administrator can read a 404 here as an absence.
 *
 ...
 *            Answers K8. Run it as a site collection administrator if you
...
(async () => {
  ...
  if (MODE === 'read') {
    ...
    record('access.lookup-acl.control-target-present-after-read', 'CONTROL: after pass 2, is the target still there, still broken, still holding the linked row?',
           'NOT ESTABLISHED',
           "answered by pass 3 (MODE = 'confirm', as the owner). Until it passes, K1, K2 "
           + 'and K3 from this pass are provisional.');
  ...
  if (MODE === 'confirm') {
    ...
    const confirm = async () => {
      ...
      // A list recreated under the same title can hold an item with the same
      // list-local id; the lookup stays bound to the original list's GUID.
      if (!field.body.LookupList || guid(field.body.LookupList) !== guid(list.body.Id)) {
        return ['FAIL', `'${LOOKUP}' is bound to list ${JSON.stringify(field.body.LookupList)}, and ${TARGET} is now ${JSON.stringify(list.body.Id)}: the target was rebuilt, so the list pass 2 met is not this one.`];
      }
      ...
      if (!linkedId) {
        return ['NOT ESTABLISHED', `the source fixture row is ${fixture ? 'present without a lookup id' : 'missing'}, so there is no linked row to look for in the target. Re-run the setup pass.`];
      }

 ...
 *   field.lookup.control-alt-column-accepted-at-creation
 *        CONTROL: a SECOND lookup created directly on the alternate column
 *        establishes that the column is a legal display target at all.
 *        Learn's `Field element (Field)` limits what may be targeted by
 *        type, so without this a refused repoint cannot be told apart from
 *        a target SharePoint would never have accepted in any form.
 *   field.lookup.control-description-merge-applies
 *        CONTROL: a Description MERGE through the SAME request envelope
 *        establishes that the request reaches this field object. Without
 *        this an ACCEPTED status with the property UNCHANGED cannot be told
 *        apart from a MERGE that applies nothing in this transport.
 ...
 *
 * TWO READINGS AN UNCONTROLLED TRANSCRIPT CANNOT SEPARATE. If the run reports
 * the MERGE ACCEPTED and the property UNCHANGED, that has two explanations and
 * they imply opposite things about whether the pin in `immutable_shape.py` is
 * justified. Either SharePoint takes the request and ignores LookupField, in
 * which case the pin is correct, or a MERGE against this field object applies
 * nothing in this transport, in which case the run measured the transport and
 * says nothing about the platform. A REFUSAL carries the mirror ambiguity: the
 * platform refusing a change, or the platform refusing a target it would never
 * have accepted at creation either. `control-description-merge-applies`
 * separates the first pair and `control-alt-column-accepted-at-creation` the
 * second. Both are measured BEFORE the LookupField MERGE is sent, and either
 * one failing voids the rows that read the repoint rather than leaving them to
 * be misread.
 *
 * THE VERBOSE TYPE IS READ, NOT ASSUMED. The verbose envelope names an entity
 * type, and a wrong type name comes back refused in exactly the shape a refused
 * property does. The probe reads `__metadata.type` off the field and prints
 * which spelling it sent beside every verdict that used it.
 *
...
(async () => {
  ...
  // The control lookup, created directly on ALT rather than repointed onto it.
  const ALT_LOOKUP = 'ProbeShowFieldAlt';
  // The Description control's sentinel, compared byte-for-byte on readback. It
  // carries this paste's own moment, so a Description left behind by an earlier
  // run can never satisfy the control without the write happening again.
  const DESC_CONTROL = `dbmlsp-probe-description-control-${Date.now()}`;
  ...
  const Q = {
    ...
    altColumn: 'CONTROL: is the alternate target column a legal lookup display target at all, measured by creating a second lookup directly on it?',
    descriptionMerge: 'CONTROL: does a MERGE of an ordinary property through this same request envelope reach this field object?',
  ...
  expect('field.lookup.control-alt-column-accepted-at-creation', Q.altColumn);
  expect('field.lookup.control-description-merge-applies', Q.descriptionMerge);
  ...
  // Every row that a failed fixture control voids rather than leaves open. The
  // two controls are in here too, because a fixture that never built leaves
  // them unmeasurable rather than merely unmeasured.
  const DEPENDANTS = [
    ['field.lookup.control-alt-column-accepted-at-creation', Q.altColumn],
    ['field.lookup.control-description-merge-applies', Q.descriptionMerge],
  ...
  // What a failed CONTROL voids: the rows that read the repoint attempt.
  // Neither control is in here, because one of them failing does not make the
  // other unanswerable, and a control that voids itself reports nothing.
  const MEASURED = DEPENDANTS.slice(2);
  const voidRows = (rows, why) => {
  ...
  log('INFO', 'probe revision d02f81da. Quote this when reporting results.');
  ...
  if (!CONFIRMED) {
    ...
    log('INFO', `first target row, and add a second lookup '${ALT_LOOKUP}' created`);
    log('INFO', `directly on '${ALT}' as a control. It would then MERGE a Description`);
    log('INFO', 'sentinel as a second control, MERGE LookupField to');
    log('INFO', `'${ALT}', and read back the field, the stored id, an item API and a`);
    log('INFO', 'rendered view.');
  ...
  // ---- Control: is the alternate column a legal display target at all? --
  // A second lookup created DIRECTLY on it, never repointed onto it. Learn's
  // `Field element (Field)` limits a lookup's display target by type, so
  // without this a refused repoint and an illegal target read identically.
  let altLookupMade = null;
  if (readFailed(await spGet(`${fieldPath(SOURCE, ALT_LOOKUP)}?$select=InternalName`))) {
    const digest = await getDigest();
    altLookupMade = await spPost(`${listPath(SOURCE)}/fields/addfield`, {
      parameters: {
        Title: ALT_LOOKUP,
        FieldTypeKind: 7,
        LookupListId: targetList.d.Id,
  ...
  const altLookupRead = await spGet(
    `${fieldPath(SOURCE, ALT_LOOKUP)}?$select=InternalName,TypeAsString,LookupField`);
  const altLookupHeld = readFailed(altLookupRead) ? null : altLookupRead.body.LookupField;
  const altLegal = altLookupHeld === ALT;
  record('field.lookup.control-alt-column-accepted-at-creation', Q.altColumn,
         altLegal ? 'PASS' : 'FAIL',
         `a second lookup '${ALT_LOOKUP}' was requested with LookupFieldName=`
         + `${JSON.stringify(ALT)}. `
         + (altLookupMade ? `Creation ${bounded(altLookupMade)}. ` : 'It already existed. ')
         + `It reads back LookupField=${JSON.stringify(altLookupHeld)}, TypeAsString=`
         + `${readFailed(altLookupRead) ? 'unreadable' : JSON.stringify(altLookupRead.body.TypeAsString)}. `
         + (altLegal
             ? `${ALT} IS a legal display target at creation, so a refusal below is `
               + 'about the CHANGE rather than about the target.'
             : `${ALT} is NOT established as a legal display target, so a refusal `
               + 'below would not be attributable to immutability.'));
  if (!altLegal) {
    voidRows(MEASURED,
             'the alternate column is not established as a legal display target, so a '
             + 'refused repoint below would not be attributable to immutability');
    return report();
  }
  ...
  // ---- Observation helpers --------------------------------------------
  // The verbose envelope names an entity type, and a wrong type name is refused
  // in exactly the shape a refused property is, so it is read off the field
  // rather than assumed to be 'SP.FieldLookup'.
  const readVerboseType = async () => {
    const response = await fetch(`${WEB}/_api/${fieldPath(SOURCE, LOOKUP)}`, {
      headers: { Accept: 'application/json;odata=verbose' },
    });
    const body = await response.json().catch(() => null);
    const type = (body && body.d && body.d.__metadata && body.d.__metadata.type) || null;
    return { ok: response.ok && Boolean(type), status: response.status, type };
  };
  const typeRead = await readVerboseType();
  const VERBOSE_TYPE = typeRead.type || 'SP.FieldLookup';
  // The fallback is never silent: every verdict that used the verbose envelope
  // prints which of the two spellings it sent.
  const typeNote = typeRead.type
    ? `verbose __metadata.type=${JSON.stringify(VERBOSE_TYPE)}, read off the field`
    : `the field's own __metadata.type could not be read (HTTP ${typeRead.status}), `
      + `so the verbose envelope fell back to ${JSON.stringify(VERBOSE_TYPE)}`;
  log('INFO', `Verbose envelope: ${typeNote}.`);

  // One envelope, two payloads. The Description control is only a control if it
  // travels exactly the way the LookupField MERGE travels.
  const mergeFieldProperty = async (payload, verbose) => {
    const digest = await getDigest();
    return verbose
      ? spPost(fieldPath(SOURCE, LOOKUP),
               { __metadata: { type: VERBOSE_TYPE }, ...payload }, digest,
               { ...MERGE_HEADERS, 'Content-Type': 'application/json;odata=verbose' })
      : spPost(fieldPath(SOURCE, LOOKUP), payload, digest, MERGE_HEADERS);
  };
  const mergeShowField = (value, verbose) =>
    mergeFieldProperty({ LookupField: value }, verbose);
  ...
  // ---- Control: does a MERGE through this envelope reach the field? ----
  // Sent BEFORE the LookupField MERGE and through mergeFieldProperty, so the
  // envelope proved here is the one the primary measurement uses, and the two
  // spellings are walked in the same order for the same reason. The sentinel is
  // left in the Description deliberately: it is this run's own evidence that
  // the transport worked, on a column this probe owns and later recycles.
  const descRead0 = await spGet(`${fieldPath(SOURCE, LOOKUP)}?$select=Description`);
  const descBefore = readFailed(descRead0) ? null : descRead0.body.Description;
  const descNoMeta = await mergeFieldProperty({ Description: DESC_CONTROL }, false);
  let descVerbose = null;
  let descRead = await spGet(`${fieldPath(SOURCE, LOOKUP)}?$select=Description`);
  let descHeld = readFailed(descRead) ? null : descRead.body.Description;
  if (descHeld !== DESC_CONTROL) {
    descVerbose = await mergeFieldProperty({ Description: DESC_CONTROL }, true);
    descRead = await spGet(`${fieldPath(SOURCE, LOOKUP)}?$select=Description`);
    descHeld = readFailed(descRead) ? null : descRead.body.Description;
  }
  const descApplied = descHeld === DESC_CONTROL;
  const descEnvelope = descApplied ? (descVerbose ? 'verbose' : 'nometadata') : null;
  const envelopeNote = descApplied
    ? `A Description MERGE through the ${descEnvelope} envelope DID apply to this `
      + 'field object in this run, so a LookupField that does not move is about the '
      + 'property rather than about the transport.'
    : 'No Description MERGE applied to this field object, so nothing here separates '
      + 'an ignored property from a request that never reached the field.';
  record('field.lookup.control-description-merge-applies', Q.descriptionMerge,
         descApplied ? 'PASS' : 'FAIL',
         `Description was ${JSON.stringify(descBefore)} before. nometadata MERGE `
         + `${bounded(descNoMeta)}`
         + (descVerbose ? `; verbose MERGE ${bounded(descVerbose)}` : '')
         + `. ${typeNote}. Description reads back ${JSON.stringify(descHeld)}, which `
         + `${descApplied ? 'is' : 'is NOT'} the sentinel ${JSON.stringify(DESC_CONTROL)}. `
         + envelopeNote);
  if (!descApplied) {
    voidRows(MEASURED,
             'no MERGE through this envelope reached the field object, so an unchanged '
             + 'LookupField below would not be attributable to the platform ignoring '
             + 'the property');
    return report();
  }
  ...
  const attempts = `nometadata MERGE ${bounded(first)}`
    + (second ? `; verbose MERGE with __metadata ${VERBOSE_TYPE} ${bounded(second)}` : '');
  ...
  // UNCHANGED is the reading the two controls exist for, so both travel with
  // it: the envelope that applied, and the legality of the target.
  record('field.lookup.showfield-property-readback', Q.readback,
         ...
         + ` ${envelopeNote} A lookup created directly on ${JSON.stringify(ALT)} holds `
         + `LookupField=${JSON.stringify(altLookupHeld)}, so that column is a legal `
         + 'display target.');
  ...
  console.log(`  2. Read the '${LOOKUP}' field. The form also carries`);
  console.log(`     '${ALT_LOOKUP}', which is the control lookup and shows ${ALT}`);
  console.log(`     whatever happened above. The subject is '${LOOKUP}'.`);

 ...
 * prevent. The control's consequence is enforced in code as of that run, and
 * AutoIndexed is now read alongside Indexed.
 *
 * The behavioural test did not run: the web's largest generic list held 21
 * items against a 5,000 threshold.
 *
...
(async () => {
  ...
  // {column: {yes: [], no: [], auto: [], unread: n}}. `auto` is tallied
  // separately from `yes` because AutoIndexed is a DIFFERENT claim from
  // Indexed and conflating them is how a platform index would get counted as
  // an author's, or the reverse.
  const tally = {};
  for (const column of SYSTEM_COLUMNS) {
    tally[column] = { yes: [], no: [], auto: [], unread: 0 };
  }
  ...
  for (const list of sample) {
    for (const column of SYSTEM_COLUMNS) {
      const field = await spGet(
        ...
        `?$select=InternalName,Indexed,AutoIndexed,TypeAsString`);
      ...
      if (field.body.AutoIndexed === true) tally[column].auto.push(list.Title);
    }
  }
  ...
  const describe = (column) => {
    ...
    return {
      ...
      evidence:
        ...
        `AutoIndexed true on ${t.auto.length}` +

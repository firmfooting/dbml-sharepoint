 ...
 *
 * HOW TO RUN: edit READS to name two lists on the site and the date columns
 * to print (the defaults are the adoption program's). F12 -> Console, paste,
 * Enter; set CONFIRMED = true and paste again. Copy the RESULTS block back.
 */
(async () => {
  // ---- Operator gate -------------------------------------------------
  ...
  // Operator-set: which lists and which date columns to print.
  const READS = [
    { list: 'ADOPT_ProgramAction', columns: ['DueDate', 'CompletedDate'], orderBy: 'Modified' },
    { list: 'ADOPT_ProgramIssue', columns: ['RaisedDate', 'ResolvedDate'], orderBy: 'Created' },
  ];

...
(async () => {
  ...
  log('INFO', 'probe revision c141cae9. Quote this when reporting results.');
  ...
  const stored = async (k) => {
    ...
    const identical = storedXml === `<Where>${where}</Where>`;
    ...
    return {
      ...
      evidence: `${k} disjunct(s); sent ${where.length} chars, stored ${storedXml.length} chars, `
        + `${identical ? 'byte-identical' : 'REWRITTEN on save'}; the stored XML replays to `

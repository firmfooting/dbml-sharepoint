 ...
 * (test/manual/multi-value-probe.js,
 * query.caml-adhoc.multichoice-includes and .multichoice-notincludes, the
 * rows once called C4 and C5, measured 2026-08-10 and
 ...
 * and returns different rows is silent. multi-value-probe.js
 * query.view-query.multichoice-membership-selects (C8) stored one
...
(async () => {
  ...
  log('INFO', 'probe revision d5d41f80. Quote this when reporting results.');
  ...
  // below is verbatim from multi-value-probe.js
  // field.multichoice.create-plain-post (M1), which is known to work.
  digest = await getDigest();
  ...
  // WHICH WRITE SHAPE, asked rather than assumed. multi-value-probe.js
  // field.multichoice.item-write-shape (M3) established `collection-metadata`
  // under odata=verbose; this harness talks
  ...
  const stored = async (k) => {
    ...
    // by multi-value-probe.js query.view-query.multichoice-chain-selects
    // (C14): `<FieldRef Name="Evt"/>` came back as `<FieldRef Name="Evt" />`. A raw string comparison is therefore false

 ...
 *        path it was asked for rather than beside its parent?
...
(async () => {
  ...
  log('INFO', 'probe revision 3937f7ae. Quote this when reporting results.');
  ...
  if (!CONFIRMED) {
    ...
    if (CLEANUP) {
      ...
      log('INFO', `CLEANUP is off: an existing '${LIB}' would be reused, and folders`);
  ...
  const orphanBack = await spGet(
    `web/GetFolderByServerRelativeUrl('${pathAt(0)}/${MISSING_PARENT}/${LEVELS[0]}')`
    + '?$select=Exists,ServerRelativeUrl');
  const folderControlHeld = !orphan.ok && isRefusal(orphan.status) && !orphanBack.ok;
  record('library.folder.control-missing-parent-refused', Q.missingParent,
         orphan.ok ? 'FAIL' : isRefusal(orphan.status)
           ? (orphanBack.ok ? 'REFUSED, BUT THE FOLDER EXISTS' : 'PASS')
           : 'NOT ESTABLISHED',
         `a folder creation under '${MISSING_PARENT}', which was never created, `
         + `returned HTTP ${orphan.status}`
         + (orphan.ok ? '' : `: ${clip(orphan.text, 220)}`)
         + `; reading the folder it would have made returned HTTP ${orphanBack.status}. `
         + (orphan.ok
           ...
           : isRefusal(orphan.status) && !orphanBack.ok
  ...
  for (let level = 0; level < LEVELS.length; level += 1) {
    ...
    const already = await spGet(
      `web/GetFolderByServerRelativeUrl('${pathAt(level + 1)}')?$select=Exists`);
    if (already.ok) {
      spellings.push(`'${name}' already present`);
      madeFolders.push({ level, spelling: 'already present', res: already });
      continue;
    }
  ...
  for (let level = 1; level <= LEVELS.length; level += 1) {
    const read = await spGet(
      `web/GetFolderByServerRelativeUrl('${pathAt(level)}')`
      + '?$select=Name,ServerRelativeUrl,Exists,ItemCount');
    levelReads.push({
      level,
      status: read.status,
      body: readFailed(read) ? null : read.body,
      at: !readFailed(read) && read.body.ServerRelativeUrl === pathAt(level),
    });
  }
  ...
  const foldersReady = colReady && levelReads.every((read) => read.at);
  record('library.folder.fixture-nested-folders-created', Q.folders,

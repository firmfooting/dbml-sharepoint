 ...
 * carries; R3 says whether a CUSTOM level is wearing the name; R3b records
 * that whether the BUILT-IN has been edited is still unanswered.
 *
 ...
 * it judges ONE thing: the RoleTypeKind SharePoint stamps on the level named
 * `Read`. It makes no comparison against any other level, on this site or
 * elsewhere, and it cannot see an ADMINISTRATOR-EDITED built-in, which is why
 * that question is a row of its own (R3b) rather than something R3 implies.
 *
 ...
 *                                                          wearing the name?
 *   access.role-def.read-level-edited-from-stock     (R3b) has the built-in
 *                                                          itself been edited?
 *                                                          Open: there is no
 *                                                          stock bitmap to
 *                                                          compare against.
 ...
 *                                                          this site have?
 *
 * All eight are `access` questions, which is the surface this probe files
...
(async () => {
  ...
  log('INFO', 'probe revision d50340c0. Quote this when reporting results.');


  // R3 reads ONE field and says so. The comment here used to claim it compared
  // this site's Read against its neighbours; it never did, and the difference
  // matters, because an administrator who edits the built-in leaves the name
  // and the type alone. That question is R3b's, and R3b is open.
  //
  // No bitmap is hardcoded either. The High/Low pair for a stock Read is a
  // 64-bit mask this project has measured on exactly one site (R2, run 1), and
  // writing one from memory is the failure AGENTS.md opens with.
  const TYPE_ONLY_NOTE = 'read from the RoleTypeKind SharePoint stamps on the level; no bitmap was compared';
  ...
  expect('access.role-def.read-level-customised', 'Is a CUSTOM level wearing the name Read on this site?');
  expect('access.role-def.read-level-edited-from-stock', 'Has this site\'s BUILT-IN Read been edited away from stock?');
  ...
  if (read) {
    ...
    record('access.role-def.read-level-customised', 'Is a CUSTOM level wearing the name Read on this site?',
      customType ? 'FAIL' : 'PASS',
      customType
        ? `RoleTypeKind=0 means SharePoint does not consider this a built-in: a CUSTOM level is wearing the name 'Read' on this site, and the deploy would bind to it. ${TYPE_ONLY_NOTE}`
        : `RoleTypeKind=${read.RoleTypeKind}, so SharePoint still regards this as the built-in Reader level. That is silent on whether the built-in has been EDITED, which is R3b. ${TYPE_ONLY_NOTE}`);

    // ---- read-level-edited-from-stock (R3b): the half R3 cannot see -----------
    // An administrator can edit a built-in level in place, and whether that
    // changes its RoleTypeKind has never been measured here, so R3 passing does
    // not rule an edit out. Deciding it needs a stock bitmap to compare
    // against, and this project has one site's measurement, which is a
    // reference point rather than a constant. Recorded open, with the pair this
    // run saw, so accumulated runs build the reference the gate would need.
    const high = read.BasePermissions && read.BasePermissions.High;
    const low = read.BasePermissions && read.BasePermissions.Low;
    record('access.role-def.read-level-edited-from-stock', 'Has this site\'s BUILT-IN Read been edited away from stock?',
      'NOT ESTABLISHED',
      `this site's Read is High=${high}, Low=${low}. Nothing compares that to a stock Read, because no stock bitmap has been measured on enough sites to be one: run 1 (2026-08-14) saw High=176, Low=138612833 on a single Team Site. Report the pair above. A second site that disagrees settles this question, and sites that agree give #209's fail-closed gate something to test against.`);
  }
  ...
  console.log('R2/R3 answer half of #199 part 2: what this site\'s Read carries,');
  ...
  console.log('R3b is the other half and stays open: nothing here can tell an');
  console.log('EDITED built-in from a stock one, for want of a measured stock');
  console.log('bitmap to compare against. Report the pair R3b prints.');

 ...
 * finish it.
 *
 * WHAT IT ASKS (console half). Ids follow the grammar in
 * `test/manual/SURFACES.md`: `<surface>.<scope>.<question>`. The old mnemonic
 * each one replaces is given beside it, because the prose below and every run
 * reported against this probe quote the mnemonics.
 *
 *   expression.client-validation.control-unknown-function-refused  (X0)
 *        NEGATIVE CONTROL: is a formula calling a function that does not
 *        exist REFUSED? If SharePoint stores nonsense, then "accepted"
 *        below means nothing and only the eyes-on half is evidence.
 *   expression.client-validation.contains-indexof          (X1)
 *        indexOf(...) >= 0            candidate for `contains`
 *   expression.client-validation.not-contains-indexof      (X2)
 *        indexOf(...) < 0             candidate for `not_contains`
 *   expression.client-validation.begins-with-indexof       (X3)
 *        indexOf(...) == 0            candidate for `begins_with`
 *   expression.client-validation.begins-with-startswith    (X4)
 *        startsWith(...)              does this function exist at all?
 *   expression.client-validation.begins-with-substring     (X5)
 *        substring(...) == '...'      documented-functions-only fallback
 *        for `begins_with`, in case X3 or X4 misbehave
 *   expression.client-validation.not-begins-with-indexof   (X6)
 *        indexOf(...) != 0            candidate for `not_begins_with`
 *
...
(async () => {
  ...
  const CANDIDATES = [
    ['expression.client-validation.contains-indexof', 'ShowContains', 'contains',
     `=indexOf([$${SUBJECT}], '${NEEDLE}') >= 0`],
    ['expression.client-validation.not-contains-indexof', 'ShowNotContains', 'not_contains',
     `=indexOf([$${SUBJECT}], '${NEEDLE}') < 0`],
    ['expression.client-validation.begins-with-indexof', 'ShowBeginsWith', 'begins_with via indexOf',
     `=indexOf([$${SUBJECT}], '${NEEDLE}') == 0`],
    ['expression.client-validation.begins-with-startswith', 'ShowStartsWith', 'begins_with via startsWith()',
     `=startsWith([$${SUBJECT}], '${NEEDLE}')`],
    ['expression.client-validation.begins-with-substring', 'ShowSubstring', 'begins_with via substring()',
     `=substring([$${SUBJECT}], 0, ${NEEDLE.length}) == '${NEEDLE}'`],
    ['expression.client-validation.not-begins-with-indexof', 'ShowNotBeginsWith', 'not_begins_with',
     `=indexOf([$${SUBJECT}], '${NEEDLE}') != 0`],
  ];
  ...
  expect('expression.client-validation.control-unknown-function-refused', 'NEGATIVE CONTROL: a formula calling a non-existent function is refused');
  ...
  // ---- control-unknown-function-refused (X0): NEGATIVE CONTROL --------

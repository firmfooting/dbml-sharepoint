 ...
 *      ITS OWN. `query.caml.tautology-conjunct-inert` on
 *      caml-chain-depth-probe.js has exactly the same blind spot, for the
 *      same reason.
 ...
 * IsNull(ID)] both changes no rows (`query.caml.tautology-conjunct-inert`)
 * and triggers the refusal (`view.filter-editor.tautology-guard-editable`).
 *
 ...
 * `<surface>.<scope>.<question>`. The prose above, and every run reported
 * against this probe, quote the old mnemonics, so both are given.
 *
 *   Q0  view.filter-editor.fixture-rows-seeded
 *   S1  query.caml.guarded-single-clause-inert
 *   S2  query.caml.tautology-alone-partitions
 *   Q1  query.view-query.guarded-single-clause-stored
 *   F1  view.filter-editor.both-pages-fetchable
 *   F2  view.filter-editor.english-markers-on-guarded
 *   F3  view.filter-editor.editable-only-ids
 *   F4  view.filter-editor.refused-only-ids
 *   F5  view.filter-editor.differing-name-attributes
 *   F6  view.filter-editor.ui-culture-readable
 *   F7  view.filter-editor.control-unfiltered-view
 *   F8  view.filter-editor.control-page-deterministic
 *   C1  view.filter-editor.pinned-control-discriminates
 *   C2  view.filter-editor.second-control-agrees
 *   C3  view.filter-editor.predicate-stable-on-refetch
 *   C4  view.filter-editor.condition-slot-count
 *   C5  view.filter-editor.sentinel-on-every-page
 *   C6  view.filter-editor.control-non-editor-page
 *   C7  view.filter-editor.trailing-markup-tokens
 *   P1  view.filter-editor.ground-truth-plain-editable
 *   P2  view.filter-editor.ground-truth-guarded-refused
 *
 * S1, S2 and Q1 file under `query`, not `view`: they ask what a CAML tree
 * returns and whether it survived storage, which is true of the tree wherever
 * it is read. SURFACES.md records the straddle. They stay distinct from
 * caml-chain-depth-probe.js's `tautology-conjunct-inert`,
 * `tautology-always-true` and `guarded-tree-stored`, which ask the same
 * questions of a twelve-clause chain on a forty-eight-member list rather than
 * of one clause on three rows.
 *
 * HOW TO RUN
...
(async () => {
  ...
  log('INFO', 'probe revision b67f4aaa. Quote this when reporting results.');
  ...
  // caml-chain-depth-probe.js (`query.caml.tautology-conjunct-inert` and
  // `view.filter-editor.tautology-guard-editable`), and S1 re-measures the
  // first here.
  ...
  expect('view.filter-editor.fixture-rows-seeded', 'the fixture actually built');
  expect('query.caml.guarded-single-clause-inert', 'does the guarded filter return the same rows as the plain one?');
  expect('query.caml.tautology-alone-partitions', 'CONTROL: does the tautology ALONE return every row?');
  expect('query.view-query.guarded-single-clause-stored', 'did the guarded tree survive being STORED as a ViewQuery?');
  expect('view.filter-editor.both-pages-fetchable', 'can the view edit page be fetched for both views?');
  expect('view.filter-editor.control-page-deterministic', 'CONTROL: is one page the same across two fetches?');
  expect('view.filter-editor.english-markers-on-guarded', 'do the English refusal markers appear on the shape #267 emits?');
  expect('view.filter-editor.editable-only-ids', 'which id attributes are on the EDITABLE page and not the refused one?');
  expect('view.filter-editor.refused-only-ids', 'which id attributes are on the REFUSED page and not the editable one?');
  expect('view.filter-editor.differing-name-attributes', 'which name attributes differ between the two pages?');
  expect('view.filter-editor.ui-culture-readable', 'can the tenant UI culture be read, so non-English can be told apart?');
  expect('view.filter-editor.control-unfiltered-view', 'CONTROL: how does a view with NO filter compare?');
  expect('view.filter-editor.pinned-control-discriminates', 'is the pinned control name absent from the refused page and present on both others?');
  expect('view.filter-editor.second-control-agrees', 'does the second candidate agree with the first?');
  expect('view.filter-editor.predicate-stable-on-refetch', 'is the predicate the same across a second fetch of all three pages?');
  expect('view.filter-editor.condition-slot-count', 'how many condition slots does the editor markup carry?');
  expect('view.filter-editor.sentinel-on-every-page', 'is there a sentinel on every page, so a failed fetch cannot read as protected?');
  expect('view.filter-editor.control-non-editor-page', 'CONTROL: does a page that is NOT the editor read as protected?');
  expect('view.filter-editor.trailing-markup-tokens', 'does the trailing markup after the final `</html>` carry a document or control token?');
  expect('view.filter-editor.ground-truth-plain-editable', 'GROUND TRUTH: does the plain view open its filter pane? (manual: look)');
  expect('view.filter-editor.ground-truth-guarded-refused', 'GROUND TRUTH: is the guarded view refused by the editor? (manual: look)');
  ...
  if (!made.ok) {
    record('view.filter-editor.fixture-rows-seeded', 'the fixture actually built', 'NOT ESTABLISHED',
  ...
  record('view.filter-editor.fixture-rows-seeded', 'the fixture actually built',
  ...
  // ---- guarded-single-clause-inert (S1): the two views must MEAN the same ---
  ...
  record('query.caml.guarded-single-clause-inert', 'does the guarded filter return the same rows as the plain one?',
  ...
  // ---- tautology-alone-partitions (S2): the tautology has to be one -----
  ...
  record('query.caml.tautology-alone-partitions', 'CONTROL: does the tautology ALONE return every row?',
  ...
  // ---- guarded-single-clause-stored (Q1): read the stored ViewQuery back ---
  ...
  record('query.view-query.guarded-single-clause-stored', 'did the guarded tree survive being STORED as a ViewQuery?',
  ...
  // ---- both-pages-fetchable (F1): is the page reachable at all? ---------
  ...
  record('view.filter-editor.both-pages-fetchable', 'can the view edit page be fetched for both views?',
  ...
  // ---- control-page-deterministic (F8): is a page even deterministic? ---
  ...
  record('view.filter-editor.control-page-deterministic', 'CONTROL: is one page the same across two fetches?',
  ...
  // ---- english-markers-on-guarded (F2): the markers on the shape #267 emits ---
  ...
  const discriminating = bothFetched
    ...
    : [];
  record('view.filter-editor.english-markers-on-guarded', 'do the English refusal markers appear on the shape #267 emits?',
  ...
  // ---- what actually differs (F3/F4/F5) ---------------------------------
  ...
  record('view.filter-editor.editable-only-ids', 'which id attributes are on the EDITABLE page and not the refused one?',
  ...
  record('view.filter-editor.refused-only-ids', 'which id attributes are on the REFUSED page and not the editable one?',
  ...
  record('view.filter-editor.differing-name-attributes', 'which name attributes differ between the two pages?',
  ...
  // ---- ui-culture-readable (F6): can non-English be told apart? ---------
  ...
  record('view.filter-editor.ui-culture-readable', 'can the tenant UI culture be read, so non-English can be told apart?',
  ...
  // ---- control-unfiltered-view (F7): the negative control ---------------
  ...
  record('view.filter-editor.control-unfiltered-view', 'CONTROL: how does a view with NO filter compare?',
  ...
  // ---- pin the candidate and try to break it (C1..C5) --------------------
  ...
  record('view.filter-editor.pinned-control-discriminates', 'is the pinned control name absent from the refused page and present on both others?',
  ...
  record('view.filter-editor.second-control-agrees', 'does the second candidate agree with the first?',
  ...
  record('view.filter-editor.predicate-stable-on-refetch', 'is the predicate the same across a second fetch of all three pages?',
  ...
  record('view.filter-editor.condition-slot-count', 'how many condition slots does the editor markup carry?',
  ...
  const onAll = threePages
    ...
    : [];
  record('view.filter-editor.sentinel-on-every-page', 'is there a sentinel on every page, so a failed fetch cannot read as protected?',
  ...
  // ---- control-non-editor-page (C6): the way an absence test fails ------
  ...
  record('view.filter-editor.control-non-editor-page', 'CONTROL: does a page that is NOT the editor read as protected?',
  ...
  // ---- trailing-markup-tokens (C7): the tail after the document close ----
  ...
  record('view.filter-editor.trailing-markup-tokens', 'does the trailing markup after the final `</html>` carry a document or control token?',
  ...
  // ---- the ground truth (P1/P2) ------------------------------------------
  record('view.filter-editor.ground-truth-plain-editable', 'GROUND TRUTH: does the plain view open its filter pane? (manual: look)',
  ...
  record('view.filter-editor.ground-truth-guarded-refused', 'GROUND TRUTH: is the guarded view refused by the editor? (manual: look)',

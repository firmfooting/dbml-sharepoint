 ...
 * pass.
 *
 * WHAT THE TEARDOWN CLEARS, and it is not only this run's writes. With
 * REMOVE_INDEXES_AT_END set, every contract column reading Indexed=true when
 * the run starts is put back to false, together with every column this run
 * indexed from unindexed. The re-paste that restores the fixture is by
 * definition a run that finds the columns already indexed, so a teardown
 * scoped to its own writes would have nothing to clear and would leave the
 ...
 * contract column reading Indexed=true, whichever run turned it on, and prints
 * which ones read false afterwards. Leaving them indexed is a choice, not a
 * default: say which it was in the transcript.
 */
...
// finding: large-list-index-teardown-clears-what-it-finds-not-what-it-wrote -
// run 2026-09-08: the REMOVE_INDEXES_AT_END re-paste reverted nothing. The
// teardown filtered to columns whose start flag was Indexed=false and which
// this run then indexed, and the whole point of that paste is that the five
// columns already read Indexed=true, so the filter matched none of them. The
// teardown now clears every contract column that reads Indexed=true at the
// start as well, which is the only version of it a re-paste can use.
// finding: large-list-index-text-number-choice-date-lookup-take - run 2026-09-08:
// on a library of 5500 files, a MERGE of Indexed:true as SP.Field held on LVText,
// LVNumber, LVChoice, LVDate and LVLookup; each read Indexed=false before and
// Indexed=true after. The plain single-value column kinds are all indexable past
// the threshold.
// finding: large-list-index-multichoice-and-calculated-refuse - run 2026-09-08:
// the same MERGE was REFUSED (HTTP 500, "This column type is not supported") on
// LVMultiChoice and on the calculated LVCalc. A multi-value choice column and a
// calculated column cannot be indexed on this surface, so a view that filters or
// groups on either past the threshold cannot be rescued with an index.
// finding: large-list-index-removes-filter-and-sort-throttle - run 2026-09-08:
// a selective filter that throttled with no index was served once the column
// took one: LVNumber eq 7 went HTTP 500 REFUSED (threshold) before to HTTP 200
// SERVED (6 rows) after Indexed:true; $orderby=LVText did the same for sort. An
// index is therefore the tool that turns a full scan into a seek on this surface.
// finding: large-list-index-is-per-column - run 2026-09-08: at one moment a
// filter on the indexed LVNumber was SERVED while a filter on the still-unindexed
// LVChoice was REFUSED (threshold). An index lifts its own column only; it is
// not a library-wide switch, so every filtered or grouped column needs its own.
(async () => {
  // ---- Operator gate -------------------------------------------------
  ...
  log('INFO', 'probe revision c11e47b7. Quote this when reporting results.');
  ...
  // indexed it or found it that way, so the fixture returns to the state a
  // before/after measurement needs. Off, because a run whose after half is
  // still waiting on an index build needs the index to still be there.
  ...
  if (!CONFIRMED) {
    ...
    log('INFO', REMOVE_INDEXES_AT_END
      ? 'REMOVE_INDEXES_AT_END is ON: every contract column that is indexed at the end'
        + ' would be put back to Indexed=false, including any a previous run left.'
  ...
  // ---- Teardown ---------------------------------------------------------
  // Two sets, and they are not the same one. What THIS run turned on is what a
  // measurement run reports leaving behind. What the teardown has to clear is
  // every contract column that is indexed at all, because the paste that
  // restores the fixture is by construction the one that finds the columns
  // already indexed: on 2026-09-08 a REMOVE_INDEXES_AT_END re-paste reverted
  // nothing, the five columns the run before it had indexed all read
  // Indexed=true at the start, and a teardown scoped to its own writes had
  // nothing in it.
  const indexedByThisRun = CANDIDATES
    ...
    .map((row) => row.field);
  const indexedOnArrival = CANDIDATES
    .filter((row) => flags[row.field].indexed === true)
    .map((row) => row.field);
  if (!REMOVE_INDEXES_AT_END) {
    ...
    if (indexedOnArrival.length) {
      log('INFO', `Still indexed from a previous run: ${indexedOnArrival.join(', ')}.`);
    }
  ...
  const toClear = CANDIDATES
    .map((row) => row.field)
    .filter((name) => indexedByThisRun.indexOf(name) !== -1
      || indexedOnArrival.indexOf(name) !== -1);
  if (!toClear.length) {
    log('INFO', 'REMOVE_INDEXES_AT_END is on and no contract column read Indexed=true, so '
      + 'there is nothing to put back.');
    return;
  }
  log('INFO', `REMOVE_INDEXES_AT_END: clearing ${toClear.join(', ')}`
    + (indexedOnArrival.length
      ? `. ${indexedOnArrival.join(', ')} arrived indexed from an earlier run and are `
        + 'cleared too, which is what the re-paste is for.'
      : '.'));
  ...
  log(stillIndexed.length ? 'FAIL' : 'OK',
      ...
      + (stillIndexed.length
        ? `. STILL INDEXED: ${stillIndexed.join(', ')}. The next run cannot take the unindexed `
          + 'half of a before/after measurement on those.'
        : '. The fixture is back to the state a before/after measurement needs.'));
})();

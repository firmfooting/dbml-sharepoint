...
(async () => {
  ...
  log('INFO', 'probe revision 6f057037. Quote this when reporting results.');
  ...
  if (madeList !== null && !madeList.ok) {
    record('field.default-formula.fixture-list-created', Q.listFixture, 'FAIL', short(madeList));
    voidAll(LIST_IDS, `fixture incomplete: list creation failed (HTTP ${madeList.status})`);
  } else {
    ...
    listHeld = await establishFixture('field.default-formula.fixture-list-created',
      () => spGet(`${listPath}?$select=BaseTemplate`), { BaseTemplate: 100 }, LIST_IDS);
  }
  ...
  const madeLib = haveLib.ok ? null : await spPost('web/lists', {
    Title: LIB, BaseTemplate: 101,
    Description: 'dbml-sharepoint guards probe library. Safe to delete.',
  }, digest);
  if (madeLib !== null && !madeLib.ok) {
    record('library.doc-lib.fixture-library-created', Q.libFixture, 'FAIL', short(madeLib));
    voidAll(LIB_IDS, `fixture incomplete: library creation failed (HTTP ${madeLib.status})`);
    return report();
  }
  ...
  if (!await establishFixture('library.doc-lib.fixture-library-created',
    () => spGet(`${libPath}?$select=BaseTemplate`), { BaseTemplate: 101 }, LIB_IDS)) {
    return report();
  }

...
(async () => {
  ...
  expect('query.caml-adhoc.fixture-dm-column', 'DM reads back as a date-only DateTime column');
  expect('field.date.fixture-td-column', 'TD reads back as a date-only DateTime column with the dynamic default [today]');
  ...
  const DM_ROWS = ['query.caml-adhoc.today-element-site-date', 'query.caml-adhoc.today-offset-element-previous-day'];
  let dmHeld = false;
  const ensureDm = async () => {
    ...
    let head = `DM ensured (${present ? 'already present' : 'created'})`;
    if (!present && !ALLOW_WRITES) {
      head = 'DM absent and ALLOW_WRITES is false, so it was not created';
    } else if (!present) {
      ...
      if (!made.ok) head = `DM create refused: HTTP ${made.status} ${made.text.slice(0, 160)}`;
    }
    // Read back on reuse as well as on create, because an earlier run may have left DM as date and time.
    dmHeld = await establishFixture('query.caml-adhoc.fixture-dm-column',
      () => spGet(`${listPath}/fields/getbyinternalnameortitle('DM')`),
      { TypeAsString: 'DateTime', DisplayFormat: 0 }, DM_ROWS);
    if (!dmHeld) return head;
  ...
  for (const row of rows) {
    if (!dmHeld && DM_ROWS.includes(row.id)) continue;
  ...
  const FILL = 'field.date.dynamic-default-rest-fill';
  const FILL_Q = 'a [today] dynamic default filled through REST';
  const TD_Q = RESULTS.find((r) => r.id === 'field.date.fixture-td-column').question;
  if (!ADD_DEFAULT_COLUMN) {
    record('field.date.fixture-td-column', TD_Q, 'NOT REACHED', 'ADD_DEFAULT_COLUMN is off, so TD was never created or read.');
    ...
    record('field.date.fixture-td-column', TD_Q, 'NOT ESTABLISHED', 'ADD_DEFAULT_COLUMN needs ALLOW_WRITES');
    ...
    log('INFO', created);
    // Read back on reuse as well as on create, because a TD left by an earlier run may carry no default.
    if (!await establishFixture('field.date.fixture-td-column',
      () => spGet(`${listPath}/fields/getbyinternalnameortitle('TD')`),
      { TypeAsString: 'DateTime', DisplayFormat: 0, DefaultValue: '[today]' }, [FILL])) {
      return report();
    }
    ...
    const id = made.ok && made.body && made.body.d ? made.body.d.Id : null;
    const back = id === null || id === undefined ? null : await spGet(`${listPath}/items(${id})?$select=Id,TD,T,Created`);
    if (!made.ok) {
      record(FILL, FILL_Q, 'NOT ESTABLISHED', `${created}; could not create the item: HTTP ${made.status} ${made.text.slice(0, 160)}`);
    } else if (back === null || readFailed(back) || !('TD' in back.body)) {
      record(FILL, FILL_Q, 'NOT ESTABLISHED',
        `${created}; the item create answered HTTP ${made.status}, but TD on the created item was never read, `
        + `so whether the default filled is unobserved${back === null ? ' (the create served no id)' : ` (HTTP ${back.status})`}`);
    } else {
      // The item carried no TD, so a value read back is the default filling and a blank is it not filling.
      record(FILL, FILL_Q, back.body.TD ? 'FILLED' : 'BLANK',
        `${created}; REST item with no dates: TD ([today]) = ${back.body.TD}; T (=TODAY() default) = ${back.body.T}; Created ${back.body.Created}. Now open New on the list and note the prefilled TD.`);
    }
  }
  return report();
})();

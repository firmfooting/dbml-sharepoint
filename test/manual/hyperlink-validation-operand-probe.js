 ...
 * beside it, because the evidence recorded against this probe quotes them.
 *
 *   formula.validation.fixture-url-column-seeded  (Q0) the probe list, its URL
 *                                                      column and a control
 *                                                      row are set up
 *   formula.validation.url-operand-accepted       (Q1) SharePoint ACCEPTS a
 *                                                      ValidationFormula
 *                                                      referencing a URL
 *                                                      column
 *   formula.validation.url-rule-refuses-violation (Q2) the rule FIRES: a
 *                                                      violating row is
 *                                                      refused
 *   formula.validation.url-rule-passes-compliant  (Q3) the rule PASSES a
 *                                                      compliant row
 *   formula.validation.url-isblank-sees-empty-description
 *                                                 (Q4) ISBLANK sees a Url with
 *                                                      an EMPTY description as
 *                                                      present
 *   formula.validation.url-equality-matches-url   (Q5) an equality comparison
 *                                                      matches the Url rather
 *                                                      than the Description
 *
 * `hyperlinkOperandVerdict` in `_hyperlink_results_v1.js.j2` still takes
 * `Q1`/`Q2`/`Q3` as its parameter names. Those are the shared reduction's own
 * argument names, not check ids, and renaming them would move a contract this
 * probe does not own.
 *
...
(async () => {
  ...
  expect('formula.validation.fixture-url-column-seeded', 'the probe list, its URL column and a control row are set up');
  expect('formula.validation.url-operand-accepted', 'SharePoint ACCEPTS a ValidationFormula referencing a URL column');
  expect('formula.validation.url-rule-refuses-violation', 'the rule FIRES: a violating row is refused');
  expect('formula.validation.url-rule-passes-compliant', 'the rule PASSES a compliant row (not simply refusing everything)');
  expect('formula.validation.url-isblank-sees-empty-description', 'ISBLANK sees a Url with an EMPTY description as present');
  expect('formula.validation.url-equality-matches-url', 'an equality comparison matches the Url rather than the Description');
  ...
  log('INFO', `probe revision d17f428c; core v2; results v1.`);
  ...
  try {
    ...
    if (!prepared.ok) {
      record('formula.validation.fixture-url-column-seeded', 'setup', 'ABORTED', prepared.error);
      throw new Error('fixture ownership or reset failed');
    }
    if (prepared.existing) {
      record(
        'formula.validation.fixture-url-column-seeded',
        'setup',
        'ABORTED',
    ...
    // === fixture-url-column-seeded (Q0): setup ============================
    ...
    if (!made.ok) {
      record('formula.validation.fixture-url-column-seeded', 'setup', 'ABORTED',
             `could not create the probe list: HTTP ${made.status} ${made.error}`);
    ...
    if (!field.ok) {
      record('formula.validation.fixture-url-column-seeded', 'setup', 'ABORTED',
             `could not add the Doc URL column: HTTP ${field.status} ${field.error}`);
      throw new Error('setup failed');
    }
    ...
    // === url-operand-accepted (Q1): will SharePoint STORE the formula? ===
    ...
    record(
      'formula.validation.url-operand-accepted',
    ...
    if (set1.ok) {
      // === url-rule-refuses-violation (Q2): does it FIRE? ===============
      ...
      record(
        'formula.validation.url-rule-refuses-violation',
      ...
      // === url-rule-passes-compliant (Q3): a compliant row? =============
      ...
      record(
        'formula.validation.url-rule-passes-compliant',
      ...
      // === url-isblank-sees-empty-description (Q4): Description empty ===
      ...
      // === url-equality-matches-url (Q5): which half is compared? =======
      ...
      if (!set2.ok) {
        record(
          'formula.validation.url-equality-matches-url',
      ...
      for (const id of [
        ...
        record(
          ...
          isSharePointBehavioralRefusal(set1)
            ? 'SharePoint refused the formula at url-operand-accepted, so the conditional downstream question has no executable subject'
            : 'url-operand-accepted failed without a semantic response '
    ...
    const verdict = hyperlinkOperandVerdict({
      Q1: q('formula.validation.url-operand-accepted'),
      Q2: q('formula.validation.url-rule-refuses-violation'),
      Q3: q('formula.validation.url-rule-passes-compliant'),
    });
    console.table(results.map(({ id, question, observed, detail }) => ({ id, question, observed, detail })));
    log(
      'VERDICT',
      `accepted=${q('formula.validation.url-operand-accepted')} `
      + `fires=${q('formula.validation.url-rule-refuses-violation')} `
      + `discriminates=${q('formula.validation.url-rule-passes-compliant')} `
      + `blank_reads=${q('formula.validation.url-isblank-sees-empty-description')} `
      + `equality_reads=${q('formula.validation.url-equality-matches-url')} `
      + `=> operand_usable=${verdict.operandUsable}`,
    );

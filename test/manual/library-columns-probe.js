 ...
 *        item-MERGE refusals observed below are unproven.
...
(async () => {
  ...
  log('INFO', 'probe revision b5380c26. Quote this when reporting results.');
  ...
  if (itemId === null) {
    record('library.column.control-missing-column-refused', 'NEGATIVE CONTROL: a MERGE naming a missing column on a file item is refused', 'NOT ESTABLISHED',
           ...
           + 'nothing to send a test write to. Item-MERGE refusals below this line are '
  ...
  if (!choiceCreated || itemId === null) {
    ...
    if (!writeChoice.ok) {
      if (isRefusal(writeChoice.status)) metadataRefusals.add('library.column.choice-column-on-library');
  ...
  if (!lookupCreated || itemId === null || targetRowId === null) {
    ...
    if (!writeLookup.ok) {
      if (isRefusal(writeLookup.status)) metadataRefusals.add('library.column.lookup-column-on-library');
      record('library.column.lookup-column-on-library', 'Does a lookup column behave the same on a document library',
  ...
  if (!reqCreated) {
    ...
    if (!reqUpload.ok) {
      ...
      const served = !readFailed(fileProps) && typeof fileProps.body === 'object'
        && !Array.isArray(fileProps.body) && 'CheckOutType' in fileProps.body;
  ...
  if (!ruleStored) {
    ...
    if (refused) {
      metadataRefusals.add('library.validation.validation-formula-on-library');
      digest = await getDigest();

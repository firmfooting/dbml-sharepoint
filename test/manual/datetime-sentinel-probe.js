 ...
 *        what time zone does the SITE run in, which of its two candidate
 *        offsets is in force, and does it match this browser? <Today/>
 *        renders in the SERVER's zone, so every same-day question below is
 *        read through this row. Not a pass/fail.
 *
 *   -- Validation formulas ------------------------------------------------
 ...
 *        so V6 reports NOT ESTABLISHED unless the site offset TZ0
 *        determined puts it exactly one site-local day ahead. On a
...
(async () => {
  ...
  expect('formula.datetime.fixture-probewhen-date-time-column', 'ProbeWhen reads back as a date-and-time DateTime column');
  // Every row that saves or queries against ProbeWhen's time of day.
  ...
  // asked.
  //
  // Bias and DaylightBias give two CANDIDATE offsets and no statement of
  // which is in force, and on the 2026-09-02 run that ambiguity alone closed
  // the gate on nine rows. So the offset is RESOLVED rather than assumed, and
  // only an unresolvable one falls back to demanding both candidates be safe.
  const tz = await spGet('web/RegionalSettings/TimeZone');
  const info = (tz.ok && tz.body && tz.body.Information) || null;
  const nowTz = new Date();
  const utcHour = nowTz.getUTCHours() + nowTz.getUTCMinutes() / 60;
  const browserOffsetMin = -nowTz.getTimezoneOffset();

  // Ask SharePoint to render a known UTC instant in the site's own zone. The
  // answer has whatever DST rule the zone is under already applied, which is
  // the thing the biases do not state.
  const siteLocalOffsetMin = async (whenUtc) => {
    ...
    for (const path of [
      ...
      if (!parts) continue;
      // The reply is a WALL CLOCK carrying no zone. Reading it through
      // Date.UTC keeps it one; parsing the string directly would apply this
      // browser's offset and answer a different question entirely.
      const wall = Date.UTC(+parts[1], +parts[2] - 1, +parts[3], +parts[4], +parts[5], +parts[6]);
      return { offsetMin: Math.round((wall - whenUtc.getTime()) / 60000), path, value };
    }
    return null;
  };
  ...
  if (info) {
    ...
    const candidates = [...new Set([standardMin, daylightMin])];
    let determined;
    if (candidates.length === 1) {
      siteOffsetsMin = candidates;
      determined = 'the zone reports no daylight bias, so its one offset needs no resolving';
    } else {
      const measured = await siteLocalOffsetMin(nowTz);
      // Accepted only when it lands on one of the zone's own two offsets. A
      // zone is on standard or daylight time and nothing else, so any other
      // answer means this call was misread, and a wrong hour here would
      // silently re-date every same-day row below.
      if (measured && candidates.includes(measured.offsetMin)) {
        siteOffsetsMin = [measured.offsetMin];
        determined = `${measured.path} rendered ${nowTz.toISOString()} as ${measured.value}, `
          + `so the offset in force is ${measured.offsetMin >= 0 ? '+' : ''}${measured.offsetMin}min`;
      } else {
        siteOffsetsMin = candidates;
        determined = 'UNRESOLVED, so the gate below falls back to both candidates: '
          + (measured
            ? `the conversion answered ${measured.value} (${measured.offsetMin}min), which is neither candidate`
            : 'SharePoint would not convert a UTC instant to site-local time')
          + `; Information reads ${JSON.stringify(info)}`;
      }
    }
    serverHours = siteOffsetsMin.map((m) => ((utcHour + m / 60) % 24 + 24) % 24);
    const desc = (tz.body.Description || '(no description)');
    tzEvidence = `site zone "${desc}"; candidate site-local offsets `
      + `${candidates.map((m) => `${m >= 0 ? '+' : ''}${m}min`).join(' / ')}; ${determined}; `
      + `this browser ${browserOffsetMin >= 0 ? '+' : ''}${browserOffsetMin}min; `
      + `site-local hour now ~${serverHours.map((h) => h.toFixed(1)).join(' or ')}`;
    const shares = siteOffsetsMin.includes(browserOffsetMin);
  ...
  // The determined site-local hour must leave room for -3h and +3h to stay on
  // the same site-local day. An unresolved offset leaves both candidates here,
  // and then both must clear it.
  ...
  const dateTimeHeld = await establishFixture(
    ...
    { TypeAsString: 'DateTime', DisplayFormat: 1 }, TIME_OF_DAY_ROWS);
  ...
  // built in the browser's zone can land on a different site-local day. TZ0
  // leaves the offset it determined here, or both candidates where it could
  // not determine one, and every entry must put the stamp exactly one
  // site-local day ahead: if both offsets say day+1, it is day+1 whichever
  // is live.
  ...
  // Named here because the eyes-on check at the end prints them.
    ...
    if (!allowance.ok) {
      ...
      if (!V6_SAFE) {
        record('formula.datetime.today-plus-one-ceiling-tomorrow-night', 'Under <= TODAY()+1, the exact ceiling (tomorrow 23:00)',
               ...
               + 'site offset TZ0 determined does not put it exactly one site-local day '
  ...
  if (!dateTimeHeld) {
    log('INFO', `ProbeWhen did not read back as a date-and-time column, so no view was made. `
                + `See the fixture row, delete '${LIST}' if it is the wrong shape, and re-run.`);
    return;
  }
  console.log('\n============ ONE EYES-ON CHECK ============');

 ...
 *         carry it. SEE THE CORRECTION BELOW, the non-vacuity was NOT
 *         established in run 1
 ...
 *
 * The code now selects Order, reads it back, and records NOT ESTABLISHED
 * when the control field did not move, so the premise is measured instead of
 * described. R3 needs a re-run before anything relies on partial MERGE for
 * role definitions. Nothing else in run 1 is affected: R1, R2, R5 and R6
 * each compare a value they themselves sent, and R7 compares the bitmap it
 * sent, so none of them depend on R3.
 */
(async () => {
  // ---- Operator gate -------------------------------------------------
  ...
  log('INFO', 'probe revision b5af03c5. Quote this when reporting results.');
  ...
  const ORDER_BEFORE = 100;
  const ORDER_AFTER = 101;
  const omitRes = await mergeDescription({ Order: ORDER_AFTER });
  if (!omitRes.ok) {
    ...
    if (readFailed(backOmit)) {
      ...
      const orderMoved = gotOrder === ORDER_AFTER;
      if (!orderMoved) {
        ...
        record('R3', 'Does a MERGE that OMITS Description preserve the previous one?',
          ...
          `the request carried Order ${ORDER_AFTER} and the level read back Order ${gotOrder}, so this MERGE cannot be shown to have done anything. A surviving Description would be equally explained by the whole request being ignored. Description read ${JSON.stringify(gotDesc)}`);
      } else {
        record('R3', 'Does a MERGE that OMITS Description preserve the previous one?',
          gotDesc === MERGED ? 'PASS' : 'FAIL',
          gotDesc === MERGED
            ? `Description survived a MERGE that did not carry it, and the same call DID move Order from ${ORDER_BEFORE} to ${gotOrder}, so the request was not ignored. Description read ${JSON.stringify(gotDesc)}`
            : `Description did NOT survive a partial MERGE; expected ${JSON.stringify(MERGED)}, read ${JSON.stringify(gotDesc)}. Order moved to ${gotOrder}, so the request was applied`);
      }
    }
  }
  ...
  if (readFailed(assignments)) {
    ...
    if (rows.length === 0) {
      ...
      record('R9', 'Can the role assignments referencing a level be enumerated?',
        'NOT ESTABLISHED (bindings not usable)',
        `read ${rows.length} row(s), but ${parsed.length} had a parseable RoleDefinitionBindings collection and none exposed an Id or Name. A gate cannot match a level against these, so the capability is unproven rather than present.`);
    } else {
      record('R9', 'Can the role assignments referencing a level be enumerated?',
        'PASS',
        `read ${rows.length} web-scope role assignment(s); ${parsed.length} had parseable bindings and ${identified} exposed a level Id or Name a gate could match on. Web-scope usage is therefore reportable. PER-LIST usage is a separate and much more expensive question this run does not answer.`);
    }
  }

 ...
 *
 *   text.role-desc.control-baseline           (N1) does the $filter probe
 *                                                  report a level that is
 *                                                  not there as absent?
 *   text.role-desc.roundtrip-create           (R1) byte-identical from CREATE
 *   text.role-desc.roundtrip-merge            (R2) byte-identical from MERGE
 *   text.role-desc.merge-partial              (R3) does a MERGE that omits
 *                                                  Description preserve it?
 *   text.role-desc.length-ceiling             (R4) what comes back at 1000+?
 *   text.role-desc.empty                      (R5) is an empty one accepted?
 *   text.role-desc.marker-shape               (R6) does the marker shape
 *                                                  lists and groups use
 *                                                  survive?
 *   access.role-def.basepermissions-readback  (R7) does BasePermissions read
 *                                                  back as written?
 *   access.role-def.getbyname-absent-status   (R8) what does getbyname
 *                                                  return for an ABSENT
 *                                                  level?
 *   access.role-def.web-assignments-enumerable (R9) can the role assignments
 *                                                  referencing a level be
 *                                                  enumerated?
 *
 * The seven text questions share their question names with
 * `list-description-probe.js` and `group-description-probe.js` on purpose,
 * and the SCOPE is what keeps the three batteries apart: a result about a
 * role definition's description is not evidence about a list description,
 * and nothing in the id should imply that it is.
 *
 * R7 to R9 file under `access`, not `text`, because a check is keyed to the
 * surface of its own question rather than the surface of its probe. A
 * permission bitmap, a lookup status code and an assignment enumeration are
 * access questions that happen to be asked by the probe that already has a
 * role definition to hand. They still depend on this probe's own
 * `text.role-desc.control-baseline`, and a dependency that crosses surfaces
 * that way is allowed.
 *
...
(async () => {
  ...
  log('INFO', 'probe revision ec198435. Quote this when reporting results.');
  ...
  expect('text.role-desc.control-baseline', 'CONTROL: does the $filter probe report a level that does not exist as absent?');
  expect('text.role-desc.roundtrip-create', 'Does a plain Description written at CREATE read back byte-identical?');
  expect('text.role-desc.roundtrip-merge', 'Does a Description written by MERGE read back byte-identical?');
  expect('text.role-desc.merge-partial', 'Does a MERGE that OMITS Description preserve the previous one?');
  expect('text.role-desc.length-ceiling', 'What length comes back when 1000+ characters go out?');
  expect('text.role-desc.empty', 'Is an EMPTY description accepted?');
  expect('text.role-desc.marker-shape', 'Does a provenance marker of the shape lists and groups use survive?');
  expect('access.role-def.basepermissions-readback', 'Does BasePermissions read back as written?');
  expect('access.role-def.getbyname-absent-status', 'What does getbyname actually return for an ABSENT level?');
  expect('access.role-def.web-assignments-enumerable', 'Can the role assignments referencing a level be enumerated?');
  ...
  if (readFailed(absent)) {
    record('text.role-desc.control-baseline', 'CONTROL: does the $filter probe report a level that does not exist as absent?',
  ...
  record('access.role-def.getbyname-absent-status', 'What does getbyname actually return for an ABSENT level?',
  ...
  if (readFailed(afterCreate)) {
    record('text.role-desc.roundtrip-create', 'Does a plain Description written at CREATE read back byte-identical?',
  ...
  if (!readFailed(afterCreate)) {
    ...
    record('access.role-def.basepermissions-readback', 'Does BasePermissions read back as written?',
  ...
  await askDescription('text.role-desc.roundtrip-merge', 'Does a Description written by MERGE read back byte-identical?',
  ...
  if (readFailed(beforeOmit)) {
    ...
    record('text.role-desc.merge-partial', 'Does a MERGE that OMITS Description preserve the previous one?',
  ...
  const longBack = await askDescription('text.role-desc.length-ceiling', 'What length comes back when 1000+ characters go out?',
    null, { Description: LONG });
  if (longBack !== null) {
    record('text.role-desc.length-ceiling', 'What length comes back when 1000+ characters go out?',
  ...
  // ---- R6: the marker shape ---------------------------------------------
  await askDescription('text.role-desc.marker-shape', 'Does a provenance marker of the shape lists and groups use survive?',
    MARKER, { Description: MARKER });

  // ---- R5: empty ---------------------------------------------------------
  await askDescription('text.role-desc.empty', 'Is an EMPTY description accepted?', '', { Description: '' });
  ...
  if (readFailed(assignments)) {
    record('access.role-def.web-assignments-enumerable', 'Can the role assignments referencing a level be enumerated?',

 ...
 *     nearest measurable substitute is `query.odata.continuation-link-emitted`
 *     and `-followed` (S11/S12) in
 ...
 *   access.principal.reader-resolves-readonly        (A1)
 *       Does the nominated account resolve READ-ONLY in this site's User
 ...
 *   access.effective-perms.control-caller-readable   (A2)
 *       CONTROL: does getusereffectivepermissions work AT ALL for the
 ...
 *   access.effective-perms.web-scope-useremoteapis   (A3)
 *       The reader's effective permissions at WEB scope, decoded into
 *       named permissions. HEADLINE OBSERVATION: is UseRemoteAPIs present?
 *       A finding only if A2 held; otherwise supporting detail.
 *   access.effective-perms.list-scope-viewlistitems  (A4)
 *       The reader's effective permissions at LIST scope for the nominated
 *       list, decoded. HEADLINE OBSERVATION: is ViewListItems present?
 *       A finding only if A2 held; otherwise supporting detail.
 *   access.effective-perms.lockdown-feature-active   (A5)
 *       Is the 'Limited-access user permission lockdown mode' site
 ...
 *
 *     access.principal.ensureuser-everyone                       (B1)
 *     access.principal.ensureuser-everyone-except-external-users (B2)
 *     access.principal.ensureuser-all-authenticated-users        (B3)
 *     access.principal.ensureuser-all-forms-users                (B4)
 *
 ...
 *
 *     access.group.members-at-top-5000                           (C1)
 *     access.group.members-no-top-next-link                      (C2)
 *     access.group.members-next-link-second-page                 (C3)
 *
 ...
 *   NOT ESTABLISHED when they are small, and read
 *   `query.odata.continuation-link-emitted` and `-followed` (S11/S12) in
 ...
 *
 * Two scope choices, since neither was obvious. A1 and B1..B4 file under
 * `access.principal`, a scope added to `test/manual/SURFACES.md` with this
 * commit: resolving a display name or a UPN to a principal is not a role
 * definition, a binding, a group or an ACL, and it is what those five rows
 * ask. A5 files under `access.effective-perms` rather than a scope of its
 * own, because lockdown mode is asked here only as the thing that strips
 * UseRemoteAPIs from Limited Access, which is A3's subject.
 *
...
(async () => {
  ...
  log('INFO', 'probe revision dc0ff4ff. Quote this when reporting results.');
  ...
  // ---- Registration ----------------------------------------------------
  expect('access.principal.reader-resolves-readonly', 'Does the nominated account resolve read-only, and to what claims LoginName?');
  expect('access.effective-perms.control-caller-readable', 'CONTROL: does getusereffectivepermissions work at all for the account running this?');
  expect('access.effective-perms.web-scope-useremoteapis', 'The reader\'s effective permissions at WEB scope: is UseRemoteAPIs present?');
  expect('access.effective-perms.list-scope-viewlistitems', 'The reader\'s effective permissions at LIST scope: is ViewListItems present?');
  expect('access.effective-perms.lockdown-feature-active', 'Is the limited-access user permission lockdown mode site collection feature active?');
  expect('access.principal.ensureuser-everyone', 'What does web/ensureuser return for the display name "Everyone"?');
  expect('access.principal.ensureuser-everyone-except-external-users', 'What does web/ensureuser return for "Everyone except external users"?');
  expect('access.principal.ensureuser-all-authenticated-users', 'What does web/ensureuser return for "All Authenticated Users"?');
  expect('access.principal.ensureuser-all-forms-users', 'What does web/ensureuser return for "All Forms Users"?');
  expect('access.group.members-at-top-5000', 'How many members does web/sitegroups(id)/users return at $top=5000, as the deploy reads it?');
  expect('access.group.members-no-top-next-link', 'With NO $top, does web/sitegroups(id)/users return a short page and a d.__next?');
  expect('access.group.members-next-link-second-page', 'Does following d.__next return a further page of DIFFERENT members?');
  ...
  {
    if (readerLogin) {
      record('access.principal.reader-resolves-readonly', 'Does the nominated account resolve read-only, and to what claims LoginName?',
  ...
  let callerLogin = null;
  {
    ...
    if (readFailed(me) || typeof me.body.LoginName !== 'string') {
      ...
      record('access.effective-perms.control-caller-readable', 'CONTROL: does getusereffectivepermissions work at all for the account running this?',
  ...
  // ---- A3: WEB scope ----------------------------------------------------
  {
    ...
    if (!readerLogin) {
      record('access.effective-perms.web-scope-useremoteapis', question, 'NOT ESTABLISHED (prerequisite)', 'the account did not resolve; see A1');
    } else {
      const r = await readEffectivePermissions('web', readerLogin);
      if (!r.ok) {
        record('access.effective-perms.web-scope-useremoteapis', question,
        ...
        if (!controlHeld) {
          record('access.effective-perms.web-scope-useremoteapis', question, 'NOT ESTABLISHED (control open)',
                 controlOpenEvidence(readerLogin, headline, describePerms(r)));
        } else {
          record('access.effective-perms.web-scope-useremoteapis', question, `OBSERVED: ${headline}`, describePerms(r));
        }
      }
    }
  }
  ...
  // ---- A4: LIST scope ---------------------------------------------------
  {
    ...
    if (PLACEHOLDER.test(LIST_TITLE)) {
      record('access.effective-perms.list-scope-viewlistitems', question, 'NOT ESTABLISHED (prerequisite)',
             'LIST_TITLE is still the placeholder. Set it to a list this bundle provisioned.');
    } else if (!readerLogin) {
      record('access.effective-perms.list-scope-viewlistitems', question, 'NOT ESTABLISHED (prerequisite)', 'the account did not resolve; see A1');
    } else {
      ...
      if (readFailed(listRes)) {
        ...
        if (!r.ok) {
          record('access.effective-perms.list-scope-viewlistitems', question,
                 r.unknownShape ? 'NOT ESTABLISHED (unrecognised response shape)' : failureOutcome(r.res),
                 r.unknownShape
                   ? `HTTP 200 but no High/Low found in ${JSON.stringify(r.res.body).slice(0, 400)}`
                   : httpDetail(r.res));
          ...
          if (!controlHeld) {
            record('access.effective-perms.list-scope-viewlistitems', question, 'NOT ESTABLISHED (control open)',
                   controlOpenEvidence(readerLogin, headline, detail));
          } else {
            record('access.effective-perms.list-scope-viewlistitems', question, `OBSERVED: ${headline}`, detail);
          }
        }
      }
    }
  ...
  // recorded as NOT ESTABLISHED and never as "lockdown is off".
  {
    ...
    if (rows === null) {
      record('access.effective-perms.lockdown-feature-active', question, 'NOT ESTABLISHED',
      ...
      record('access.effective-perms.lockdown-feature-active', question,
  ...
  // having answered everything it can.
  {
    ...
    if (PLACEHOLDER.test(GROUP_NAME)) {
      ...
      record('access.group.members-at-top-5000', C1_Q, 'NOT ESTABLISHED (prerequisite)', why);
      record('access.group.members-no-top-next-link', C2_Q, 'NOT ESTABLISHED (prerequisite)', why);
      record('access.group.members-next-link-second-page', C3_Q, 'NOT ESTABLISHED (prerequisite)', why);
    } else {
      ...
      if (!Number.isInteger(groupId)) {
        ...
        if (all === null) {
          ...
          record('access.group.members-at-top-5000', C1_Q, `NOT ESTABLISHED (HTTP ${allRes.status})`, why);
          record('access.group.members-no-top-next-link', C2_Q, 'NOT ESTABLISHED (prerequisite)', why);
          record('access.group.members-next-link-second-page', C3_Q, 'NOT ESTABLISHED (prerequisite)', why);
        } else {
          const allNext = allRes.body.d.__next;
          record('access.group.members-at-top-5000', C1_Q, `OBSERVED: ${all.length} member(s)`,
          ...
          if (firstPage === null) {
            ...
            record('access.group.members-no-top-next-link', C2_Q, `NOT ESTABLISHED (HTTP ${firstRes.status})`, why);
            record('access.group.members-next-link-second-page', C3_Q, 'NOT ESTABLISHED (prerequisite)', why);
            ...
            const why = `the no-$top read returned ${firstPage.length} row(s), which is every member C1 `
              ...
              + '`query.odata.continuation-link-emitted` and `-followed` (S11/S12) in '
              + 'search-discovery-probe.js.j2, which pages a list-items collection against an '
              + 'existing 6,000-row fixture. Do NOT read this row as "the endpoint does not page".';
            record('access.group.members-no-top-next-link', C2_Q, 'NOT ESTABLISHED (group not larger than the server page size)', why);
            record('access.group.members-next-link-second-page', C3_Q, 'NOT ESTABLISHED (prerequisite)', why);
          } else {
            ...
            record('access.group.members-no-top-next-link', C2_Q,
            ...
            if (!present) {
              record('access.group.members-next-link-second-page', C3_Q, 'NOT ESTABLISHED (prerequisite)',
              ...
              if (page2 === null) {
                record('access.group.members-next-link-second-page', C3_Q,
  ...
  // ================= GROUP B: WRITE-GATED ==============================
  const CLAIM_NAMES = [
    ['access.principal.ensureuser-everyone', 'Everyone'],
    ['access.principal.ensureuser-everyone-except-external-users', 'Everyone except external users'],
    ['access.principal.ensureuser-all-authenticated-users', 'All Authenticated Users'],
    ['access.principal.ensureuser-all-forms-users', 'All Forms Users'],
  ];

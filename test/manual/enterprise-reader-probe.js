 ...
 *
 * STATUS: RUN ONCE, 2026-08-11, on ONE live site. Groups A and C ran with
 * the write flag OFF. GROUP B DID NOT RUN and every question in it is
 * still unmeasured.
 *
 *   RUN:      2026-08-11, probe revision 0e565ebc, pasted into ONE
 *             Microsoft 365 group-connected Team Site — NOT a publishing
 *             site. Read-only: ALLOW_WRITES was off.
 *   FINDINGS: A1-A5 below, in full. C1 held. C2 and C3 AS THEY WERE
 *             WRITTEN FOR THAT RUN WERE AN INVALID EXPERIMENT and have
 *             been rewritten since; see GROUP C below for what was wrong
 *             and what replaced it. B1-B4 NOT ESTABLISHED (write flag).
 *
 ...
 *     It can only be answered by SIGNING IN AS the service account. The
 *     operator's own session cannot answer it, and this probe runs in the
 *     operator's session, so no revision of this file can close it.
 *   - The same A3/A4 measurement on a PUBLISHING site.
 *   - GROUP B entirely: the two tenant-wide claim encodings Learn does not
 *     publish. Needs ALLOW_WRITES = true on a disposable site.
 *   - Server-driven paging on a site group's members: see GROUP C.
 *
...
(async () => {
  ...
  log('INFO', 'probe revision 71e7b796 — quote this when reporting results.');

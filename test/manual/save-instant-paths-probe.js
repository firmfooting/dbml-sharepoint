...
(async () => {
  ...
  if (MODE === 'setup') {
    ...
    // Count saves only after readback; accepted writes can leave their rows unobserved.
    const outcomes = [];
    let attempted = 0;
    let saved = 0;
    let refused = 0;
    ...
    record('formula.validation.today-default-races-modified-rule-rest', 'five bare REST creates: does the [today] default race the rule?',
      ...
      `${attempted} attempted; ${saved} saved by readback; ${refused} refused; `
      + (answered ? outcomes.join(' | ')
        : `${attempted - saved - refused} of the five creates neither saved nor were refused: ${outcomes.join(' | ')}`));
    ...
    if (CREATE_HIDDEN_LIST) {
      ...
      const read = hb !== null && !readFailed(hb)
        && typeof hb.body === 'object' && !Array.isArray(hb.body) && 'Hidden' in hb.body;

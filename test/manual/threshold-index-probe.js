 ...
 * sends the UNINDEXED twin, whose answer on a list this size is expected to
 * come back SHORT of the complete one; a short answer is recorded as that
 * negative control and NOT as a guard comparison, because two truncated
 * results agreeing establishes nothing about the conjunct. GRDONLY sends the
 * guard with no filter at all, matching every row in the list, and must be
 ...
 *
 * THIS PROBE WRITES. Two lists, six columns, four indexes, six views, and the
 ...
 *     created-date-counts-as-join   (JOINCRT)  does the Created DATE column count
 *                                              as a join? (issue #53)
 *     modified-date-counts-as-join  (JOINMOD)  does the Modified DATE column count
 *                                              as a join? (issue #53)
 ...
 *
 *   NOT ESTABLISHED: Created, Modified, Author and Editor as FILTERS. All four
 *   were refused at 6,000, which this fixture cannot read as evidence: the
 *   loader owns every row, so each matches the whole list and breaches on
 *   result-set size. Needs rows from several principals over several intervals.
 *   This is a statement about the four `scale.native-idx` rows and about
 *   nothing else. The `scale.join` rows PROJECT these columns at the ceiling
 *   rather than filtering on them, and a projection does not care who authored
 *   the rows or when, so JOINCRT and JOINMOD are unaffected by it.
 *
...
(async () => {
  ...
  log('INFO', 'probe revision 54e4ec65. Quote this when reporting results.');
  ...
  expect('scale.threshold.fixture-lists-created', 'Both probe lists are generic lists (BaseTemplate 100)');
  expect('scale.index.fixture-columns-typed', 'Every probe column reads back as its declared type, and Parent looks up the parent list');
  ...
  expect('scale.join.created-date-counts-as-join', 'Does the Created DATE column count as a join?');
  expect('scale.join.modified-date-counts-as-join', 'Does the Modified DATE column count as a join?');
  ...
  const LISTS_ROW = 'scale.threshold.fixture-lists-created';
  const COLUMNS_ROW = 'scale.index.fixture-columns-typed';
  ...
  if (!await establishFixture(LISTS_ROW, async () => {
    const body = {};
    for (const [key, title] of [['parent', PARENT], ['main', LIST]]) {
      const read = await spGet(`web/lists/getbytitle('${odata(title)}')?$select=BaseTemplate`);
      if (unanswered(read) !== null) return read;
      body[`${key}.BaseTemplate`] = read.body.BaseTemplate;
    }
    return { ok: true, status: 200, body };
  }, { 'parent.BaseTemplate': 100, 'main.BaseTemplate': 100 },
  ...
  for (const [name, , schemaXml] of schemas) {
    if (readFailed(await readField(name))) {
      const made = await addField(schemaXml);
      if (!made.ok) log('FAIL', `Could not create ${name}: HTTP ${made.status} ${made.text.slice(0, 300)}`);
    }
  }

  // A column reused by name is read back before any index is set, since an earlier run may have left another type.
  const guid = (value) => String(value || '').replace(/[{}]/g, '').toLowerCase();
  const declaredColumns = {};
  for (const [name, , schemaXml] of schemas) {
    declaredColumns[`${name}.TypeAsString`] = /Type="([^"]+)"/.exec(schemaXml)[1];
  }
  declaredColumns['Parent.LookupList'] = (value) => guid(value) !== '' && guid(value) === guid(parent.Id);
  if (!await establishFixture(COLUMNS_ROW, async () => {
    const body = {};
    for (const [name] of schemas) {
      const read = await readField(name);
      if (unanswered(read) !== null) return read;
      body[`${name}.TypeAsString`] = read.body.TypeAsString;
      if (name === 'Parent') body['Parent.LookupList'] = read.body.LookupList;
    }
    return { ok: true, status: 200, body };
  }, declaredColumns, RESULTS.map((r) => r.id).filter((id) => id !== LISTS_ROW && id !== COLUMNS_ROW))) {
    report();
    return;
  }

  for (const [name, indexed] of schemas) {
    if (!indexed) continue;
  ...
  if (blocked) {
    for (const [id, question] of [
      ...
      ['scale.join.created-date-counts-as-join', 'Does the Created DATE column count as a join?'],
      ['scale.join.modified-date-counts-as-join', 'Does the Modified DATE column count as a join?'],
  ...
  // Nothing documents the Created and Modified DATE columns either way, and
  // `analysis/joins.py` excludes them by INFERENCE from their type rather than
  // from a measurement. JOINCRT and JOINMOD settle that (issue #53).
  //
  ...
  if (!available.length) {
    ...
    //
    // Created and Modified are the DATE columns, not the person-typed Created
    // By and Modified By two rows above them. `analysis/joins.py` excludes them
    // from join cost by inference from their type, which is the last inferred
    // claim in the join model (issue #53). This is a PROJECTION at the ceiling,
    // so the single-principal limitation that blocks the filter-side question
    // against this fixture does not reach it: the loader owning every row
    // decides which rows a filter matches and decides nothing about what a view
    // can project.
    ...
    const suspects = [
      ...
      ['scale.join.created-date-counts-as-join', 'Does the Created DATE column count as a join?', 'Created'],
      ['scale.join.modified-date-counts-as-join', 'Does the Modified DATE column count as a join?', 'Modified'],
    ...
    for (const [id, question, column] of suspects) {
      ...
      // Whether the column came back is printed beside the verdict and never
      // folded into it: a view that renders without returning the column under
      // test is a different reading from one that returns it.
      const returned = r.ok ? (r.present(column) ? 'PRESENT' : 'ABSENT') : 'n/a';
      record(id, question, r.ok ? 'NOT COUNTED' : 'COUNTED',
             `[${stamp}] ${worked} lookup(s) at the ceiling + ${column}, HTTP ` +
             `${r.status}, ${column} ${returned} in the returned row ` +
             `(${r.keys} key(s)). ${r.ok
  ...
  const measureGuard = async () => {
    ...
    // Rows, row IDENTITIES and status, judged by nothing. The caller compares.
    const rawCaml = async (where) => {
      ...
      //
      // ViewFields asks for ID and nothing else, so every row arrives with an
      // identity the caller can compare. Both halves of every pair send the
      // same ViewFields, so it cannot tilt a comparison.
      const viewXml = `<View><ViewFields><FieldRef Name='ID'/></ViewFields>`
        + `<Query>${where}</Query><RowLimit>${count + 1000}</RowLimit></View>`;
      const r = await spPost(
        `web/lists/getbytitle('${odata(LIST)}')/RenderListDataAsStream`,
        { parameters: { ViewXml: viewXml } }, digest);
      const returned = (r.ok && r.body && Array.isArray(r.body.Row)) ? r.body.Row : null;
      // WHICH rows, not only how many. Two queries can return the same COUNT
      // over different rows, and a comparison that sees counts alone reports
      // that as the guard having changed nothing.
      //
      // null the moment any row arrives without a readable ID, so an
      // unreadable identity fails closed instead of comparing equal.
      const ids = (returned
        && returned.every((row) => row.ID !== undefined && row.ID !== null))
        ? returned.map((row) => String(row.ID)).sort()
        : null;
      // Observed, never interpreted. Microsoft documents no response shape for
      // RenderListDataAsStream (Learn, searched 2026-08-27), so whether a
      // truncated answer carries a next-page marker is itself unknown, and
      // printing what came back is what a later run can read.
      const paging = (r.ok && r.body)
        ? `NextHref=${r.body.NextHref === undefined ? '(absent)' : JSON.stringify(r.body.NextHref)}`
          + `, FirstRow=${r.body.FirstRow}, LastRow=${r.body.LastRow}`
        : '(no body)';
      return {
        ok: r.ok, status: r.status, rows: returned ? returned.length : null,
        ids, paging, text: (r.text || '').slice(0, 200),
      };
    };
    ...
    const guardPair = async (id, question, leaf, expectServed) => {
      ...
      const shape = `[${stamp}] unguarded: HTTP ${bare.status}, ${bare.rows} `
        + `row(s) (${bare.paging}). guarded: HTTP ${wrapped.status}, `
        + `${wrapped.rows} row(s) (${wrapped.paging}). `;
      if (bare.ok !== wrapped.ok || bare.rows !== wrapped.rows) {
        record(id, question, 'CHANGED', shape
      ...
      if (!bare.ok) {
        record(id, question, 'UNCHANGED (both refused)', shape
          + 'The conjunct changed neither the outcome nor the row count on a list '
          + `of ${count}, past the ${LIST_VIEW_THRESHOLD} threshold.`);
        return;
      }
      // Equal counts are not the same answer. A conjunct that swapped WHICH
      // rows came back would read as UNCHANGED on counts alone, and that is
      // the #272 hazard in its quietest form.
      if (bare.ids === null || wrapped.ids === null) {
        record(id, question, 'NOT ESTABLISHED', shape
          + 'At least one row came back with no readable ID, so the two answers '
          + 'cannot be compared by identity and matching counts prove nothing.');
        return;
      }
      if (bare.ids.join(',') !== wrapped.ids.join(',')) {
        record(id, question, 'CHANGED (same count, different rows)', shape
          + 'The counts matched and the ROWS did not, so the conjunct changed '
          + 'which items SharePoint served. No row count would have shown it.');
        return;
      }
      record(id, question, 'UNCHANGED (both served)', shape
        + 'The conjunct changed neither the outcome, the row count nor the row '
        + `identities on a list of ${count}, past the ${LIST_VIEW_THRESHOLD} `
        + 'threshold.');
    };
    ...
    //
    // Compared by row IDENTITY rather than by count, because this asks whether
    // the twins match the same ROWS and two seeds can drift into holding the
    ...
    // back HTTP 200 holding exactly 5,000 of 5,614 on 2026-08-17. Two
    // truncated answers agreeing is not a guard-equivalence measurement,
    // however alike they look, so a short answer here is recorded as the
    ...
    if (guardBlocked) {
      ...
      const bare = await rawCaml(`<Where>${UNI_LEAF}</Where>`);
      const wrapped = await rawCaml(guarded(UNI_LEAF));
      const shape = `[${stamp}] unguarded: HTTP ${bare.status}, ${bare.rows} `
        + `row(s) (${bare.paging}). guarded: HTTP ${wrapped.status}, `
      ...
      if (!bare.ok || twinComplete === null) {
        ...
        record('scale.threshold.guarded-comparison-unindexed-text', GRDUNI_Q,
          ...
          + 'guarded half cannot be compared with it: both are partial results, '
          + 'and two partial results agreeing says nothing about the conjunct. '
          + 'Read GRDIDX knowing this list IS being truncated at this size.');
      } else {
        // Served whole, so the threshold is not biting and this row controlled
        ...
        record('scale.threshold.guarded-comparison-unindexed-text', GRDUNI_Q,
          ...
          + 'fact about a list nothing throttled. For the record, the guarded '
          + `half returned ${sameRows ? 'the same rows' : 'DIFFERENT rows'}.`);
      }
    }
    ...
    // Create one probe view and hand back a URL ONLY if the view SharePoint
    // now holds is the one this run asked for. Re-pasting is the normal way
    // to run this file, so the create failing because the title is taken is
    // expected; what must not happen is an operator being sent to whatever a
    // previous revision left behind and that observation being recorded
    ...
    const openableView = async (id, shape, question, where) => {
      ...
      // No $select: an unrecognised property name fails the WHOLE request, and
      // this needs the stored settings rather than a list of names guessed off
      // a documentation page.
      const listing = await spGet(`${listPath}/views`);
      const view = ((!readFailed(listing) && listing.body.value) || [])
        .find((v) => v.Title === title) || null;
      if (!view || !view.ServerRelativeUrl) {
        record(id, question, 'NOT ESTABLISHED',
          `[${stamp}] no view named '${title}' could be created, and none of that `
          + `title was found either: create returned HTTP ${made.status} `
          + `${(made.text || '').slice(0, 160)}`);
        return null;
      }
      // Whitespace and quote characters are normalised before comparing,
      // because SharePoint's own storage form for a ViewQuery is not
      // documented and this probe has not measured it. Anything left over is
      ...
      if (Number(view.RowLimit) !== 100) differs.push(`RowLimit is ${view.RowLimit}`);
      if (view.Paged !== true) differs.push(`Paged is ${view.Paged}`);
      if (view.PersonalView !== false) differs.push(`PersonalView is ${view.PersonalView}`);
      if (differs.length) {
        record(id, question,
          made.ok
            ? 'NOT ESTABLISHED (SharePoint stored a view other than the one sent)'
            : 'NOT ESTABLISHED (a view of this title already exists and differs)',
          `[${stamp}] '${title}' is not what this run asked for: ${differs.join('; ')}. `
          + `This run sent ViewQuery '${where}'. The create returned HTTP `
          + `${made.status}, so the view was `
          + `${made.ok ? 'stored differently from what was sent' : 'left over from an earlier run'}. `
          + 'Delete it and re-paste rather than reading it.');
        return null;
      }
      return `${window.location.origin}${view.ServerRelativeUrl}`;
    };
    ...
    for (const [id, shape, question, where] of VIEW_SHAPES) {
      ...
      // row itself what the row is still waiting for.
      record(id, question, 'MANUAL (unobserved)',
        `[${stamp}] OPEN ${url} and report ONE of: "rows" (it listed items) or `
        + '"threshold" (it showed the list view threshold message). Until '
        + 'somebody looks, this question is open. The filter is ' + where);
    }

    // ---- What does a NEGATED filter cost in the FILTER EDITOR? ----------
    // analysis/checks/_views.py warns an author when a filter renders more
    // comparisons than the editor can hold, and it counts one row per rendered
    // <FieldRef> (condition_rendering.caml_condition_count). That is a count
    // of the CAML, not of the editor. CAML has no bare <Not>, so a `neq`
    // renders as <Or><IsNull/><Neq/></Or>: two FieldRefs for one clause the
    // author wrote, and whether the editor then shows two rows, one, or
    // refuses to open the filter at all has never been measured.
    //
    // It cannot be reasoned out either. #272 rests on the editor refusing a
    // filter whose right child is a GROUP, and a negated clause puts a group
    ...
    //
    // Double-quoted and folded left, so what is sent is byte-identical to what
    // condition_rendering.to_caml emits for the same three clauses. CAML's
    // <And> is binary, which is why the fold is there at all.
    const EDT_VALUES = ['A', 'B', 'C'];
    const eqLeaf = (v) => `<Eq><FieldRef Name="Bucket"/><Value Type="Text">${v}</Value></Eq>`;
    const neqLeaf = (v) => `<Or><IsNull><FieldRef Name="Bucket"/></IsNull>`
    ...
    for (const [id, shape, question, leaf, fieldRefs] of EDITOR_SHAPES) {
      ...
      record(id, question, 'MANUAL (unobserved)',
        ...
        + 'of: the NUMBER of filter rows the editor shows, or "refused" (it would '
        + `not open the filter at all). ${EDT_VALUES.length} clause(s) here render `
        + `${fieldRefs} <FieldRef>, which is ${fieldRefs} editor row(s) by the count `
        + `_views.py uses. The filter is ${where}`);
    }
  };
  ...
  report();
  // report() counts a row open when its outcome STARTS WITH one of the open
  // prefixes, so a qualified 'NOT ESTABLISHED (...)' and an unobserved MANUAL
  // both stay open. What it cannot see is a run whose every row is void for
  // one shared reason, so say that after the summary, which is the last thing
  // on screen and the thing that gets pasted back.
  if (blocked) {

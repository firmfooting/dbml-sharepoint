 ...
 *
 *   GROUPS. For the Choice column the four labels are known exactly, so the
 *   instrument looks for a leaf element whose trimmed text IS one of them, and
 *   quotes the enclosing row so a reader can see whether a count sits beside
 *   it. For the Number column the labels are integers, which no text scan can
 *   tell from any other integer on the page, so that state's label test is not
 *   run and is said not to be run. Both states also report how many elements
 *   carry `aria-expanded`, which is an ARIA convention for a collapsible
 *   region rather than a SharePoint marker.
 *
 ...
 *      holding FEWER than 5,000 items, let it finish rendering, then paste.
 *      This is the capture control and it comes first on purpose.
 ...
 *
 * STATUS: NOT YET RUN. Authored against the merged findings of #472, #478,
 * #479, #480, #481 and #483. Nothing below has been observed on a live site,
 * and every finding line is inherited or about method until it has.
 */
...
// library under the threshold and every rendered row depends on it.
...
// fixture's own file-name stem, which no UI update can rename.
...
(async () => {
  ...
  log('INFO', 'probe revision 0866fdde. Quote this when reporting results.');
  ...
  //   1  CONTROL. Paste on a rendered library page holding FEWER than 5,000
  //      items. Establishes that this browser renders a modern library at all.
  ...
  // A group header is looked for as a LEAF element whose trimmed text IS one of
  // the labels, and the enclosing row is quoted so a reader can see whether a
  // count sits beside it. `labels` is null where the column's values are bare
  // integers and no text scan can tell one from any other number on the page.
  const groupScan = (labels) => {
    ...
    if (labels === null) {
      return { labels: null, found: [], missing: [], rows: [], counted: [],
    ...
    for (const label of labels) {
      ...
      for (const node of document.querySelectorAll('*')) {
        ...
        if (String(node.textContent || '').trim() !== label) continue;
        hit = node;
        break;
      }
      ...
      found.push(label);
      let holder = hit;
      for (let up = 0; up < 5 && holder.parentElement; up += 1) {
        holder = holder.parentElement;
        if (holder.getAttribute && holder.getAttribute('role') === 'row') break;
      }
      const text = clip(String(holder.textContent || '').trim(), 120);
      rows.push(`${label}: ${text}`);
      // A count beside the label, quoted rather than parsed into a number: what
      // the header shows is the observation, and reading it as an integer would
      // discard the form it was shown in.
      const digits = text.replace(label, '').match(/\d[\d,]*/);
      if (digits) counted.push(`${label}=${digits[0]}`);
    }
    const missing = labels.filter((l) => !found.includes(l));
    return { labels, found, missing, rows, counted, expanderCount, expanders,
      detail: `labels found ${JSON.stringify(found)}, missing ${JSON.stringify(missing)}; `
        + `enclosing rows ${JSON.stringify(rows)}; counts beside the labels ${JSON.stringify(counted)}` };
  };
  ...
  if (STATE === 0) {
    ...
    log('INFO', `  STATE = 1  a document library on this site holding FEWER than ${THRESHOLD} items.`);
  ...
  if (STATE === 1) {
    ...
    const usable = smallOk && !onFixture && Number.isFinite(items) && items < THRESHOLD;
    ...
    record('library.large-list.control-ui-modern-renders-below-threshold',
           ...
           (!usable
             ? `this page is not a library under the threshold: list id ${show(listId)}, `
               + `ItemCount ${show(smallOk ? small.body.ItemCount : null)}, is the fixture library `
               + `${show(onFixture)}. Open a document library on this site holding fewer than `
               + `${THRESHOLD} items and paste again.`
  ...
  if (STATE === 3) {
    ...
    const rendered = groups.found.length >= 2;
    ...
    record('library.large-list.ui-group-by-indexed-column-renders',
           ...
           !readable ? voidWhy
             : `${groups.detail}. ${groups.expanderCount} element(s) carry aria-expanded, showing `
               ...
               + `(a collapsed group shows none, so zero here is expected). banner phrases `
               ...
               + (banner.matched.length
                 ...
                 : rendered
                   ...
                   : maybe
                     ? 'Collapsible regions are on the page but none of the four labels was found as an '
                       + 'element of its own, so what rendered is not established from the DOM. Read the '
                       + 'screenshot and report which it is.'

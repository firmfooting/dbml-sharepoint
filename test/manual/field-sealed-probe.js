...
(async () => {
  ...
  log('INFO', 'probe revision ce49094f. Quote this when reporting results.');
  ...
  if (madeList !== null && !madeList.ok) {
    record('field.sealed.fixture-list-created', Q.fixture, 'FAIL', short(madeList));
    voidAll(AFTER_LIST, `fixture incomplete: list creation failed (HTTP ${madeList.status})`);
    return report();
  }
  ...
  if (!await establishFixture('field.sealed.fixture-list-created',
    () => spGet(`${listPath}?$select=BaseTemplate`), { BaseTemplate: 100 }, AFTER_LIST)) {
    return report();
  }
  ...
  // The seal was measured on a Text column, so a reused column of another type is not the subject.
  const usable = subject !== null && subject.TypeAsString === 'Text' && subject.Sealed === false
    && subject.Hidden === false && subject.FromBaseType === false;
  record('field.sealed.fixture-subject-column', Q.subject, usable ? 'PASS' : 'FAIL',
         `POST fields answered ${short(madeSubject)}; the field reads `
         + `TypeAsString=${show(subject === null ? undefined : subject.TypeAsString)}, ${describeField(subject)}`

...
def test_a_tool_owned_group_gets_the_plain_marker() -> None:
    """The shared groups span families, so naming one family would be false."""
    assert marker_for_group("dbml Enterprise Readers", "risk-register") == SHARED_MARKER
    assert marker_for_group("dbml List Administrators", "incident-management") == SHARED_MARKER
...
def test_both_shapes_start_with_the_detection_prefix() -> None:
    ...
    assert SHARED_MARKER.startswith(MARKER_PREFIX)
...
def test_the_composed_description_is_note_then_marker() -> None:
    ...
    assert composed == (
        "Staff who rate risks. Provisioned by dbml-sharepoint from risk-register."
    )

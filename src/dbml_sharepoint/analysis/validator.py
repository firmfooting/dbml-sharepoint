...
def _check_column(
    ...
    if col.default is not None and typemap.is_multi_value(col.type):
        ...
        findings.append(Finding(
            ...
            f"{table}.{name}: default: is not supported on a multi-value "
    ...
    if col.unique and typemap.is_multi_value(col.type):
        ...
        findings.append(Finding(
            ...
            f"{table}.{name}: [unique] is not supported on a multi-value "
            f"column -- SharePoint cannot enforce unique values on a "

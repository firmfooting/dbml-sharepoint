...
FINDING_HELP: dict[FindingCode, str] = {
    ...
    FindingCode.ATTACHMENTS_ON_DOCUMENT_LIBRARY: (
        ...
        "`EnableAttachments = false` on every list it provisions, and a "
        "library refuses a write to that property. Measured 2026-09-06 by "
        "`library.doc-lib.attachments-sticks` in the list-settings probe: "
        "writing `EnableAttachments = true` to a scratch document library "
        "came back HTTP 500, `-2146232832, "
        "Microsoft.SharePoint.SPException`, \"Attachments are not allowed for "
        "Document libraries and Surveys\", and the flag still read back "
        "false. The list side of the same run settled the other way (wrote "
        "false over true, read back false). So a library reaching the deploy "
        "would stop the paste at that list rather than accept the write "
    ...
    FindingCode.BLANK_KEY_TOOK_DEFAULT: (
        ...
        "`major_version_limit`, a view's `row_limit`, an entity's `title`, the "
        "default permission policy's `site_role` or a policy's `assignments`. "
        "The author may have meant a value. Remove the "
    ...
    FindingCode.PERMISSION_LEVEL_REDEFINES_A_BUILTIN: (
        ...
        "permission level. SharePoint itself permits a duplicate name -- "
        "`built-in-levels-probe.js` renamed a custom level to `Read` on a "
        "live site on 2026-09-06 and the site accepted it, with `getbyname` "
        "still resolving to the built-in -- but the deploy does not create "
        "a second level beside it: it probes for a role definition of that "
        "name and, finding the site's own, MERGEs the declared description "
        "and base permissions onto it. That reconciliation is deliberate -- "
        "it stops a drifted custom level silently keeping edit rights -- "
        "but pointed at a built-in it rewrites `Read` (or whichever) for "
        "EVERY principal on the site that holds it, not just the ones this "

    ...
    if bundle.mapping.display_name_mode is not None:
        for entity_name, cols in bundle.mapping.display_name_overrides.items():
            ...
            for col_name, display_title in cols.items():
                # Title takes an override like any other rendered column. It
                # was refused between #426 and 2026-09-07, because the deploy
                # never applied one and the view width map, the form body
                # section field list and the Power Query rename all did, so a
                # bundle named a field the list did not have. The patch now
                # carries the display title (MEASURED: test/manual/
                # title-rename-probe.js), so the four halves agree and the
                # ordinary length and collision rules below are what govern it.

...
| F23 | Every seam except an unresolved *No owner* picks the evidence rows it rests on in **Evidence in conflict**: at least one row, and for *Contradicts document* one *Document* row and one row of another **Type**, both with **Contradicts** ticked, that bear on the same **Bears on** column. At least one picked row bears on the column the type is about: *Single person* or *Out of hours* for *Single person*; *Dependency* or *Documentation* for *Undocumented dependency*; *Run by* or *Decided by* for a resolved *No owner* | A formula cannot read a multi-value lookup |

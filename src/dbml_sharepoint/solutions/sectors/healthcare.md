...
| Legal compliance SAQs issued by a compliance portal | legal-compliance-register | One library holds vendor-issued SAQs and supporting REGs. Filename periods identify vendor issues. The platform owner assigns an executive, the executive assigns a business owner, and the business owner completes the SAQ in Excel. Executive review is optional; the platform owner uploads the result to the compliance platform and records the handoff |
...
| M365 asks heard by champions during the rollout | digital-innovation-log | Route before score; a clinical or data review tier follows the data-sensitivity flag; nothing patient-identifiable enters the list |
...
| What the service depends on its ICT providers for | seam-register | A six-week exercise for a service that gets its ICT from a shared-services body or several providers: one row per service with evidence behind it, every disagreement as a seam, and a one-page map that is a view rather than a drawing |

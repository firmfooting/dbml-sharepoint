...
3. Walk the streams in the order they reported. For each one: a status, and a
   note **only if the status is not Green**. A Green with no note is a
   complete answer.
4. **Overall status**: your call on the day. It is not calculated. If nothing
   is wrong with any stream but the day was still hard, say Amber and say why
   in *Other updates*.
5. Anything worth calling out goes in **Recognition**. Use it. It is the
   cheapest thing on the board and the one people remember.
6. Save.
...
Do not go Green to avoid a conversation. The grid is read across a fortnight,
and a Green the day before a Red reads worse than two honest Ambers.

## When to escalate rather than absorb

Raise it on `TH_Escalation` when any of these is true:

- It cannot be resolved at this tier with the people in the room.
- It has been Amber at this tier for **three consecutive days** without
  moving.
- It needs a decision, money or a person the tier does not control.
- It affects another team, department or site.

Filling in an escalation takes about a minute: a title stating the **problem**
(not the solution), the date and tier it was raised at, the stream, the
direction, the tier being asked to act, **one named owner**, and a due date.
Never "the team" as an owner.

The direction matters: *Escalated up* asks a higher tier to act, *Delegated
down* hands work to a lower tier, *Referred sideways* passes it to a peer.
The **By route** view groups the queue by that journey, which is how you spot
a tier that only ever sends things up.

## Between huddles

- Your queue is the **Open** view filtered to you, and the **Overdue** view
  is the one your chair reads out.
...
- Do not backfill a week of boards from memory on a Friday. An honest gap is
  worth more than an invented fortnight.
- Do not edit yesterday's status because today's story changed. Yesterday was
  what it was; today gets its own row.
- Do not delete rows. The history is the point.

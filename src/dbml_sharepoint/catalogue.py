...
#: choice -- and a good many shipped families carry one of these. A rightwards
#: arrow cannot be encoded by cp1252, cp850 OR cp437, so picking a template on
#: a legacy Windows console could raise `UnicodeEncodeError` from inside rich.
#: An em dash fails on the two OEM pages but not on cp1252, which is why the
#: rule is ASCII rather than any one code page.
#:

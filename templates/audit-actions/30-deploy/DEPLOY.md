...
- [ ] **Closure needs BOTH now** (`list_validation:`, two chained rules
      sharing one message, because SharePoint gives a list a single
      validation formula). With Status *Closed* and EvidenceUrl empty,
      saving is refused. With EvidenceUrl filled and ClosedDate empty, it
      is refused again — `DaysLate` is computed from that date, so without
      it the committee's lateness figure is simply blank. Fill both; it
      saves.
